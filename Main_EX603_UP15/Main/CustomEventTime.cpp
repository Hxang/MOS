...
void CCustomEventTime::DrawEventTimePanelWindow()
{
	...
	// ----
	//Central.PrintDropBox(StartX, StartY, MainWidth, MainHeight, 0, 0);

	gCentral.gDrawWindowCustom(StartX, StartY, MainWidth, MainHeight, eEventTimePANEL_MAIN, "Bảng Sự Kiện Ingame");
	...
	if (this->EventTimeEnable == 1)
	{
		...
		for (int i = 0; i <26; i++)
		{
			...
			if (pCheckMouseOver(StartX + 10, StartY + 62 + (12 * i), Return2X(TransForX(380)), ReturnY(TransForY(12))))
			{
				pDrawColorButton(0x7980, StartX + 10, StartY + 64 + (12 * i), 350, 11, NULL, NULL, Color4f(43, 237, 5, 100));
			}
		}
