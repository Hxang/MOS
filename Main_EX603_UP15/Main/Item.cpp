...
{
	static DWORD ITEMID;
	static DWORD Addr1_JMP = 0x004EAE35;
	static DWORD Addr2_JMP = 0x004EAE6C;
	...
	if (ITEMID >= 6664 && ITEMID <= 6665
		...
		|| ITEMID >= 6826 && ITEMID <= 6845 //Idexx 13,170 - Đến 13,300 Hiển Thị Dòng Excl

		|| ITEMID >= 6856 && ITEMID <= 6956 //Idexx 13,170 - Đến 13,300 Hiển Thị Dòng Excl
...
void __declspec(naked) NewItemPedan()
{
	...
	if (ITEMID >= 6668 && ITEMID <= 6669
		|| ITEMID == 6846 //ID Pedan
		|| ITEMID == 6847 //ID Pedan
		|| ITEMID == 6848 //ID Pedan
		|| ITEMID == 6849 //ID Pedan
		|| ITEMID == 6850 //ID Pedan
		|| ITEMID == 6851 //ID Pedan
		|| ITEMID == 6852 //ID Pedan
		|| ITEMID == 6853 //ID Pedan
		|| ITEMID == 6854 //ID Pedan
		|| ITEMID == 6855 //ID Pedan
...
void PreviewCharSet(int ObjectIndex, BYTE * CharSet, lpViewObj Object, int Mode)
{
	...
	if (CustomWings > 0)
	{
		lpObj->WingsSlot = ITEM2(12, 150) + (CustomWings - 1);
	}
}
...
bool IsCustomWings(WORD ItemID, bool Preview)
{
	...
	if (ItemID >= ITEM(12, 150) && ItemID <= ITEM(12, 255))
	{
		return true;
	}
...
bool IsCustomItem(WORD ItemID, bool Preview)
{
	...
	if (ItemID >= ITEM(0, 1) && ItemID <= ITEM(11, 255))
	{
		return true;
	}
...
char* GetItemName(int ItemType, int Level)
{
	...
	//g_Console.AddMessage(1, "%d LV %d %s", ItemType, GET_ITEMOPT_LEVEL(Level), ItemNameGet);
