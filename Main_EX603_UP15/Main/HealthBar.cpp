...
void DrawNewHealthBar() // OK
{
	...
	if (HPMonsterName == 1)
	{
		for (int n = 0; n < MAX_MAIN_VIEWPORT; n++)
		{
			...
			if (!lpTarget || (lpTarget->m_Model.ObjectType != emMonster && lpTarget->m_Model.ObjectType != emPlayer) || lpTarget->aIndex != lpObj->aIndex)
			{
				isTarget = 0;
			}
			if (!isTarget)
			...
			if (*(BYTE*)(ViewportAddress + 0x320) == 1) //Check User Type
			{
				...
				pDrawBarForm((float)(PosX - 3), (float)(PosY + 1.5), LifeBarWidth + 3.0f, 10.0f, 0.0f, 0);
				pGLSwitchBlend();
				pGLSwitch();
			...
			else
			{
				pSetBlend(true);
				glColor4f((GLfloat)0.0, (GLfloat)0.0, (GLfloat)0.0, (float)0.8);
				pDrawBarForm((float)(PosX - 3), (float)(PosY + 1.5), LifeBarWidth + 3.0f, 5.0f, 0.0f, 0);
				pGLSwitchBlend();
				pGLSwitch();
				glColor3f(1.0, 1.0, 1.0);
				pSetBlend(false);

				//gTextColor.SetTextColor(255, 255, 255, 255); // blanco
				//gTextColor.CUIPaintFont((float)PosX + 35, (float)PosY, name, 0, 8);

				gCustomRankUser.DrawFormat(eWhite, PosX, PosY - 10, (int)LifeBarWidth, 3, "%s", name);

				gCustomRankUser.DrawFormat(eWhite, PosX + 52, PosY - 2, (int)LifeBarWidth, 3, "%d%%", PercentualMonster);
			...
			{
				pDrawBarForm((float)(i * 8 + PosX - 1.5), (float)(PosY + 2), 8.0, 3.5, 0.0, 0);
			}
			...
			if (ShieldPercent > 0)
			{
				...
				{
					pDrawBarForm((float)(i * 8 + PosX - 1.5), (float)(PosY + 7.0), 8.0, 3.5, 0.0, 0);
				}
			}
