...
void CRegisterForm::GenerateCaptcha()
{
	...
	memcpy(this->reCaptcha, newCaptcha, 3);
...
void CRegisterForm::Create(BYTE* thisa)
{
	...
	sprInputBoxCaptcha->Init(g_hWnd, 60, 14, 3);
	//--
...
void CRegisterForm::RenderForm()
{
	...
	g_pRenderText.RenderText(x - 10, y + 20, strTipText, 190, 0, RT3_SORT_CENTER);
	//--
	g_pRenderText.SetFont(g_hFixFont);
	//--
	g_pRenderText.SetTextColor(CLRDW_WHITE);
	g_pRenderText.RenderText(x - 32, y + 62, GlobalText(GlobalLine, 450), 90, 0, RT3_SORT_RIGHT);
	g_pRenderText.RenderText(x - 32, y + 82, GlobalText(GlobalLine, 451), 90, 0, RT3_SORT_RIGHT);
	g_pRenderText.RenderText(x - 32, y + 99, "E-mail", 90, 0, RT3_SORT_RIGHT);
	g_pRenderText.RenderText(x - 32, y + 117, "7 số bí mật", 90, 0, RT3_SORT_RIGHT);
	g_pRenderText.RenderText(x - 32, y + 136, "Mã Capcha", 90, 0, RT3_SORT_RIGHT);
	g_pRenderText.SetFont(g_FontBold);
	g_pRenderText.RenderText(x + 80, y + 136, this->reCaptcha2);
	//--
	sprInputBox->Render();
...
void CRegisterForm::RequestRegister(LPVOID pClass)
{
	...
	szth->sprInputBoxCaptcha->GetText(szCaptcha, 3 + 1);
	...
	{
		szth->CodeAction = CREATE_ACCOUNT_FAIL_CAPTCHA;
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Vui lòng nhập mã capcha!");
	}
	else if (strcmp(szth->reCaptcha, szCaptcha) != NULL)
	{
		szth->CodeAction = CREATE_ACCOUNT_FAIL_CAPTCHA;
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Mã Capcha không đúng!");
	}
	else
	{
		if (CheckName(szID))
...
void CRegisterForm::RequestAccount(BYTE code)
{
	...
	case CREATE_ACCOUNT_FAIL_ID:
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Tên người dùng đã được đăng ký");
		break;
	case CREATE_ACCOUNT_SUCCESS:
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Đăng Ký Tài Khoản Thành Công");
		break;
	case CREATE_ACCOUNT_FAIL_RESIDENT:
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "hệ thống tạo tài khoản tạm thời không khả dụng");
		break;
	default:
