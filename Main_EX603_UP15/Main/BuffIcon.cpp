...
void sub_777FB0(int BuffIndex, int IndexTime, float RenderX, float RenderY)
{
	...
	//Fix Buff Interface 97/99/S2/S6
...
void BuffIcon::sub_777EB0(float XIcon, float YIcon)
{
	...
	float Height = (float)((28.0f / 100) * 60.5);
	float Width = (float)((20.0f / 100) * 60.5);
	float RenderW = (float)((256.0f / 100) * 60.5);
	x = 0.0;
	y = 0.0;
	v27 = 0;
	v28 = 0;
	...
	for( IndexTime = 0 ; IndexTime < MAX_EFFECT_LIST; ++IndexTime)
	{
		...
		x = XIcon + v27 * (Width + 5.0) + 5.0;
		y = YIcon + v28 * (Height + 5.0) + 5.0;
...
void BuffIcon::GC_BuffInfo(PMSG_SEND_BUFF_ICON_EX* aRecv)
{
	...
	if (continuar == true)
	{
		...
		{
			//memcpy(RenderInfoBuff[BuffIndex], it->second.InfoBuff, 1aRecv->iValue[0]00); //show texxt
			wsprintf(RenderInfoBuff[BuffIndex], it->second.InfoBuff, aRecv->iValue[0]);
		}
