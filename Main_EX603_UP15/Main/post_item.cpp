...
void PostItem::PostItemToWorldChatChannelImp(void * item_data)
{
	...
	else if (GetKeyState(VK_RBUTTON) & 0x8000 && gInterface.Data[eWindowDungLuyenItem].OnShow
...
#endif
#if(SAUDOIITEM)
	else if (GetKeyState(VK_RBUTTON) & 0x8000 && gInterface.Data[eWindowDoiItem].OnShow && GetTickCount() >= gPostItem.last_tickcount_moveitem_ + 300)// Send Item Rao Ban
	{
		ObjectItem* lItem = (ObjectItem*)item_data;
		...
		pMsg.Target = -1;
		//gInterface.DrawMessage(1, "%d", lItem->ItemID);
		int JCWinWidth;
...
#endif
#if (CongHuongV2)
	//Send Item Cong Huong
	else if (GetKeyState(VK_RBUTTON) & 0x8000 && gInterface.Data[eWindow_CongHuong].OnShow
		&& GetTickCount() >= gPostItem.last_tickcount_moveitem_ + 300)// Send Item Rao Ban
	{
		ObjectItem* lItem = (ObjectItem*)item_data;
