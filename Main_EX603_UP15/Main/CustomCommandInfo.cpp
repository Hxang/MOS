...
void CCustomCommandInfo::DrawCommandWindow()
{
	...
	//--
	float WindowW = 380.0f;
	float WindowH = 250;
	//--
	//gCentral.gDrawWindowCustom(StartX, StartY, WindowW, WindowH, eCommand_MAIN, "Bảng Sử Dụng Lệnh Nhanh:");
	gCentral.gDrawWindowCustom(StartX, StartY, MainWidth, MainHeight, eCommand_MAIN, "Lệnh Cơ Bản Trong Game");

	DWORD Color = eGray100;
	// ----

	gInterface.DrawFormat(eGold, StartX + 35, StartBody + 47, 52, 1, "Lệnh:");

	gInterface.DrawFormat(eGold, StartX + 125, StartBody + 47, 52, 1, "Chức Năng:");

	gInterface.DrawFormat(eGold, StartX + 210, StartBody + 47, 52, 1, "Thông Tin:");
	...
	for (int i = (25 * Page) + 0; i < (25 * Page) + 25; i++)
	{
		...
		gInterface.DrawFormat(Color, (int)StartX + 18, (int)StartBody + 60 + (x * 10), 100, 1, this->m_CustomCommandInfo[i].Command);

		pDrawColorText(this->m_CustomCommandInfo[i].Description, (int)StartX + 112, (int)StartBody + 60 + (x * 10), 100, 0, Color, 0, 1);

		pDrawColorText(this->m_CustomCommandInfo[i].infoCommand, (int)StartX + 200, (int)StartBody + 60 + (x * 10), 180, 0, Color, 0, 1);
