...
bool CProtect::ReadPetEffectBMD(char* name) // OK
{
	...
	{
		((BYTE*)&this->m_ReadBMD)[n] += (BYTE)(0x85 ^ HIBYTE(n)); //Thay 2 cái này khi update ver :) - Đồng bộ với GetMainnInfo
...
bool CProtect::ReadMainFile(char* name) // OK
{
	...
	{
		((BYTE*)&this->m_MainInfo)[n] += (BYTE)(0x95 ^ HIBYTE(n));/// ở đây này con đĩ
...
void CProtect::CheckLauncher() // OK
{
	...
	//if (FindWindow(0, gProtect.m_MainInfo.LauncherName) == 3)
	//{
	//	//int result = system(gProtect.m_MainInfo.FileLauncherName);
	//	ExitProcess(0);
	//}
...
void CProtect::CheckHackFile() // OK
{
	...
	{
		sprintf(ErrorBuff, "SharkGuard %s CRC error!", this->m_MainInfo.HackName);
	...
	{
		sprintf(ErrorBuff, "SharkGuard %s CRC error!", this->m_MainInfo.HackName);
		MessageBox(0, ErrorBuff, "Error SharkGuard", MB_OK | MB_ICONERROR);
		ExitProcess(0);
	}
...
void CProtect::CheckMaxGameInstances() // OK
{
   ...
   //if (IsProcessRunning("Main.CheatGuard", gMaxGameInstances))
   if (IsProcessRunning("Main.exe", gProtect.m_MainInfo.MaxGameInstances))
   {
