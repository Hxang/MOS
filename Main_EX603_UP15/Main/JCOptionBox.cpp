...
void JCOptionBox::LoadAction(int Pointer, int Action)
{
	...
	{
	case JCHIDEPLAYERNAME:
	{
							 DisablePlayerName = this->JCActive;
							 if (JCActive == 0)
							 {
								 //Show All Name Player
								 SetByte(0x81C03FA, 0x01);
							 }
							 else
							 {
								 SetByte(0x81C03FA, 0x00);
							 }
	}
		break;
	...
	case JCMouseEffect:
	{
						  MouseEffect = this->JCActive;
						  if (JCActive == 0)
						  {
							  SetCompleteHook(0xE8, 0x006E89A9, 0x009CFB8D);
							  SetCompleteHook(0xE8, 0x00712729, 0x0074CD30);
							  SetCompleteHook(0xE8, 0x005B768F, 0x006D8A30);
							  SetCompleteHook(0xE8, 0x005B770C, 0x006D9070);
						  }
						  else
						  {
							  MemorySet(0x006E899D, 0x90, 0x05);
							  MemorySet(0x007126FA, 0x90, 0x05);
							  MemorySet(0x005B768F, 0x90, 0x05);
							  MemorySet(0x005B770C, 0x90, 0x05);
						  }
	}
		break;
	case JCZenDrop:
	{
					  ReduceZen = this->JCActive;
					  if (JCActive == 0)
					  {
						  SetCompleteHook(0xE8, 0x005F8924, 0x0095A1E0);
					  }
					  else
					  {
						  MemorySet(0x005F8924, 0x90, 0x05); //Reduce Drop Zen
					  }
	}
		break;
	case JCDistance:
	{
					   Distance = this->JCActive;
					   if (JCActive == 0)
					   {
