...
//		if (gInterface.IsWorkZone(eMenu))
//		{
//			//gInterface.DrawToolTip(146 + 37.2, 50, "Menu - F8");
//
//			// ----
//			if (gInterface.Data[eMenu].OnClick)
//			{
...
void OffHelper(DWORD id, float x, float y, float w, float h)
{
	...
	pDrawGUI(id, x, y, w, h);
			...
			gInterface.DrawFormat(eGold, x + 50 + 16, y + 12, 60, 1, "Treo Offline");
...
char FixHelperMouseClick(int a1){
	if (gInterface.CheckWindow(ChaosBox) || gInterface.CheckWindow(Party) || gInterface.CheckWindow(Warehouse))
		return 0;
...
bool CheckCursor(int x, int y, int w, int h)
{
	...
	return Check = pCheckMouseOver(100, 40, 40, 30);
}
