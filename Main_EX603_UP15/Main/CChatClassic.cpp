	...
	//Chat giữa
			__declspec(naked) void ChatWindowDisplay() // OK
		{
			...
			__asm
			{
					...
					test al, al
					Mov Dword Ptr Ds : [Ecx + 0x124], 190 // tọa độ đây đúng k
...
void CChatClassic::Load()
{
	//if (gProtect.m_MainInfo.CustomInterfaceType == 1||gProtect.m_MainInfo.CustomInterfaceType == 2	)
	//{//Chat giữa
	//MemorySet(0x007867B8, 0x90, 2); //-> Disable check open chat for F5
	//SetCompleteHook(0xE9, 0x0078B0BC, &ChatWindowDisplay); //-> Center chat text display
	//}
