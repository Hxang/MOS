...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_DOIITEM)
	{
		gSauChangeItem.SetStateInterface(lpObj->Index);
	...
	LogAddConnect(LOG_USER,"[Obj][%d] Out Character (%s)",lpObj->Index,lpObj->Name);
...
void CObjectManager::CharacterGameCloseSet(int aIndex,int type) // OK
{
...
#if(FIXVT == 1)
		//- Fix Switch character al morir [FIX2]
		if (lpObj->DieRegen != 0)
		{
	...
	if (gServerInfo.m_DisableResetAutoOutGame == 1)
	{
	//Del auto reset
	...
	{
		gParty.DelMember(lpObj->PartyNumber, aIndex);
	}
...
bool CObjectManager::CharacterGetRespawnLocation(LPOBJ lpObj) // OK
{
	...
	else if (gBattleRoyale.CheckPlayerTarget(lpObj))
	{
		result = gBattleRoyale.GetUserRespawnLocation(lpObj, &gate, &map, &x, &y, &dir, &level);
	}
#endif
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	while(change != 0)
	{
		...
		//--
#if(SLOTPET)
		gCustomPet.CalcCustomPetOption2(lpObj, 1);
#endif

		gCustomWing.CalcItemCustomOption(lpObj, 1);
		//--
		//=== Active BonusSkin
		gBCustomSkinModel.ActiveBonusSkin(lpObj, 1);
#if(DANHHIEU_NEW)
		gDanhHieu.DANH_HIEU_OPTION(lpObj, 1);
#endif

#if(QUANHAM_NEW)
		gQuanHam.QUAN_HAM_OPTION(lpObj, 1);
#endif
#if(TULUYEN_NEW)
		gTuLuyen.TU_LUYEN_OPTION(lpObj, 1);
#endif
	...
	//--
#if(SLOTPET)
	gCustomPet.CalcCustomPetOption2(lpObj, 0);
#endif
	gCustomWing.CalcItemCustomOption(lpObj, 0);
	//-----------------------
	//=== Active BonusSkin
	gBCustomSkinModel.ActiveBonusSkin(lpObj, 0);
#if(DANHHIEU_NEW)
	gDanhHieu.DANH_HIEU_OPTION(lpObj, 0);
#endif

#if(QUANHAM_NEW)
	gQuanHam.QUAN_HAM_OPTION(lpObj, 0);
#endif

	//-----------------------
#if(TULUYEN_NEW)
	gTuLuyen.TU_LUYEN_OPTION(lpObj, 0);
#endif
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
...
#if(B_HON_HOAN)
	lpObj->CapDoHonHoan = lpMsg->CapDoHonHoan;
#endif
	...
	//Kalima
	else if (KALIMA_MAP_RANGE(lpObj->Map) != 0) //Move về lorencia khi thoát game vào lại
	{
		result = gGate.GetGate(17, &gate, &map, &x, &y, &dir, &level);
	}

	//Chien Trường Cổ
	else if (lpObj->Map == 97) //Move về lorencia khi thoát game vào lại 
	{
		result = gGate.GetGate(17, &gate, &map, &x, &y, &dir, &level);
	}

	//Sky
	else if (lpObj->Map == MAP_ICARUS) //Move về lorencia khi thoát game vào lại 
	{
		result = gGate.GetGate(22, &gate, &map, &x, &y, &dir, &level);
	}

	//Sky
	else if (lpObj->Map == gThanMaChien.EventMap) //Move về lorencia khi thoát game vào lại 
	{
		result = gGate.GetGate(17, &gate, &map, &x, &y, &dir, &level);
	}
	...
	LogAdd(LOG_RED, "[Tổng Nạp] %d Mốc Đã Nhận (%d)", lpObj->TongNap, lpObj->NhanMocNap);
#endif
	...
	lpObj->SDAutoRecuperationTime = GetTickCount();
	lpObj->CharSaveTime = GetTickCount();
	//==CTC Lay Du lieu tu DB
	lpObj->CTCTime = lpMsg->CTCTime;
	lpObj->CTCRegDay = lpMsg->CTCRegDay;
	lpObj->UserSkinPick = lpMsg->mUserSkinPick;
