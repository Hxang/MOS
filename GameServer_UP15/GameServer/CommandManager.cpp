...
bool CCommandManager::ManagementCore(LPOBJ lpObj, char* message, int Npc) // OK
{
	...
	case COMMAND_SETPASS:
		...
		break;
#endif
	case COMMAND_RESET_VIP:
		Result = CommandResetVip(lpObj, argument, Npc);
		break;
...
bool CCommandManager::CommandMove(LPOBJ lpObj, char* arg) // OK
{
	...
	if (gBAntihack.CheckUserMove(lpObj) == 0)
	{
		return 0;
	}
...
bool CCommandManager::CommandResetVip(LPOBJ lpObj, char* arg, int Npc) // OK
{
	...
	if (lpObj->Interface.use != 0 || lpObj->State == OBJECT_DELCMD || lpObj->DieRegen != 0 || lpObj->Teleport != 0 || lpObj->PShopOpen != 0 || lpObj->SkillSummonPartyTime != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(90)); //Bạn không thể sử dụng / reset tại thời điểm này"

		if (Npc >= 0)
		{
			GCChatTargetNewSend(lpObj, Npc, gMessage.GetMessage(90)); //Bạn không thể sử dụng / reset tại thời điểm này"
		}
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(91), gResetTableVip.GetResetLevel(lpObj));			// "Bạn phải đạt ít nhất mức %d để sử dụng / reset"

		if (Npc >= 0)
		{
			GCChatTargetNewSend(lpObj, Npc, gMessage.GetMessage(91), gResetTableVip.GetResetLevel(lpObj));		// "Bạn phải đạt ít nhất mức %d để sử dụng / reset"
		}
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(92), gResetTableVip.GetResetMoney(lpObj));	//Bạn phải có ít nhất %d zen để sử dụng / reset"

		if (Npc >= 0)
		{
			GCChatTargetNewSend(lpObj, Npc, gMessage.GetMessage(92), gResetTableVip.GetResetMoney(lpObj));	//Bạn phải có ít nhất %d zen để sử dụng / reset"
		}
		return 0;
	}
	...
	if (lpObj->Reset >= gServerInfo.m_CommandResetLimit[lpObj->AccountLevel])
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(93), gServerInfo.m_CommandResetLimit[lpObj->AccountLevel]);	//"Bạn không thể có nhiều hơn %d reset"

		if (Npc >= 0)
		{
			GCChatTargetNewSend(lpObj, Npc, gMessage.GetMessage(93), gServerInfo.m_CommandResetLimit[lpObj->AccountLevel]);	//"Bạn không thể có nhiều hơn %d reset"
		}
		return 0;
	}
	...
	if (gServerInfo.m_CommandResetCheckItem[0] != 0)
	{
		for (int n = 0; n < INVENTORY_WEAR_SIZE; n++)
		{
			if (lpObj->Inventory[n].IsItem() != 0)
			{
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(94)); //"Bạn không thể thiết lập lại với các mục được trang bị"

				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj, Npc, gMessage.GetMessage(94)); //"Bạn không thể thiết lập lại với các mục được trang bị"
				}
				return 0;
			}
		}
	...
	// them GP reset
	if (lpObj->Coin1 < gResetTableVip.GetResetTienTeWCoinC(lpObj)) //WWC
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Ban thieu %d WCoinC de Reset", gResetTableVip.GetResetTienTeWCoinC(lpObj));
		return 0;
	}

	if (lpObj->Coin3 < gResetTableVip.GetResetTienTe(lpObj)) //GP
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Ban thieu %d GoblinPoint de Reset", gResetTableVip.GetResetTienTe(lpObj));
		return 0;
	}
	...
	if (gResetTableVip.GetResetTienTeWCoinC(lpObj) > 0){ //WC
		GDSetCoinSend(lpObj->Index, -gResetTableVip.GetResetTienTeWCoinC(lpObj), 0, 0, "");
	}

	if (gResetTableVip.GetResetTienTe(lpObj) > 0){ //GP
		GDSetCoinSend(lpObj->Index, 0, 0, -gResetTableVip.GetResetTienTe(lpObj), "");
	}
...
void CCommandManager::CommandResetVipAuto(LPOBJ lpObj, char* arg, int Npc) // OK
{
	...
	if (lpObj->AutoResetVipEnable == 0)
	{
		...
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(137));

		if (Npc >= 0)
		{
			GCChatTargetNewSend(lpObj, Npc, gMessage.GetMessage(137));
		}
	}
	else
	{
		...
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(138));

		if (Npc >= 0)
		{
			GCChatTargetNewSend(lpObj, Npc, gMessage.GetMessage(138));
		}
	}
...
void CCommandManager::CommandResetVipAutoProc(LPOBJ lpObj) // OK
{
	if (lpObj->Interface.use != 0 || lpObj->State == OBJECT_DELCMD || lpObj->DieRegen != 0 || lpObj->Teleport != 0 || lpObj->PShopOpen != 0 || lpObj->SkillSummonPartyTime != 0)
	{
		return;
	...
	if (lpObj->Coin1 < gResetTableVip.GetResetTienTeWCoinC(lpObj))
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(139), gResetTableVip.GetResetTienTeWCoinC(lpObj));
		return;
	}

	// Reset GP
	if (lpObj->Coin3 < gResetTableVip.GetResetTienTe(lpObj))
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(140), gResetTableVip.GetResetTienTe(lpObj));
		return;
	}
	...
	if (gResetTableVip.GetResetTienTeWCoinC(lpObj) > 0){ //WCoinC
		GDSetCoinSend(lpObj->Index, -gResetTableVip.GetResetTienTeWCoinC(lpObj), 0, 0, "");
	}
