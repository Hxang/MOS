		...
		while (true)
		{
			...
			while (true)
			{
				if (section == 0)
				{
					...
					Items.LevelMin = lpMemScript->GetAsNumber();

					Items.LevelMax = lpMemScript->GetAsNumber();
			...
			//if (lpItem->m_Index == GET_ITEM(it->ItemType, it->ItemIndex)) /*|| lpItem1->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))*/
		{
			if ((lpItem->m_Level >= it->LevelMin) && (lpItem->m_Level <= it->LevelMax) || (lpItem1->m_Level >= it->LevelMin) && (lpItem1->m_Level <= it->LevelMax))
			{
				if (lpItem->m_NewOption >= it->ItemOption || lpItem1->m_NewOption >= it->ItemOption)
				{
					...
					//lpObj->MagicDamageMin += (it->DameRate *lpObj->MagicDamageMin) / 100;//
					//lpObj->MagicDamageMax += (it->DameRate *lpObj->MagicDamageMax) / 100;//
					//lpObj->PhysiDamageMinLeft += (it->DameRate *lpObj->PhysiDamageMinLeft) / 100;//
					//lpObj->PhysiDamageMinRight += (it->DameRate *lpObj->PhysiDamageMinRight) / 100;//
					//lpObj->PhysiDamageMaxLeft += (it->DameRate *lpObj->PhysiDamageMaxLeft) / 100;//
					//lpObj->PhysiDamageMaxRight += (it->DameRate *lpObj->PhysiDamageMaxRight) / 100;//
