...
bool CCustomAttack::GetAttackSkill(LPOBJ lpObj, int* SkillNumber, int SetSkill) // OK
{
	...
	case CLASS_FE:
		...
		break;
	case CLASS_MG:
		if (lpObj->Inventory[0].m_Index >= GET_ITEM(0, 0) && lpObj->Inventory[0].m_Index < GET_ITEM(0, 512))
		{
...
void CCustomAttack::OnAttackMonsterAndMsgProc(LPOBJ lpObj) // OK
{
	...
	if (this->m_CustomAttackTime[lpObj->AccountLevel] == 0) //Disable when vip over
	{
		this->OnAttackClose(lpObj);
