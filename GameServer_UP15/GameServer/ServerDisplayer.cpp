...
CServerDisplayer::CServerDisplayer() // OK
{
	...
	this->m_brush[2] = CreateSolidBrush(RGB(0, 152, 239));	
	this->m_brush[3] = CreateSolidBrush(RGB(255, 255, 255));
	this->m_brush[4] = CreateSolidBrush(RGB(210, 210, 210));
#else
...
void CServerDisplayer::Init(HWND hWnd) // OK
{
	...
	gLog.AddLog(1, "LOG\\LOG_DOI_ITEM");
	gLog.AddLog(1, "LOG\\LOG_DUNGLUYEN");
	gLog.AddLog(1, "LOG\\LOG_NGAN_HANG_NGOC");
	gLog.AddLog(1, "LOG\\LOG_CONG_HUONG");
	gLog.AddLog(1, "LOG\\LOG_DANH_HIEU");
	gLog.AddLog(1, "LOG\\LOG_TU_LUYEN");
...
void CServerDisplayer::SetWindowName() // OK
{
	...
	{
		wsprintf(buff, "[%s] %s (ON: %d) %s   (License Free) %s", GAMESERVER_VERSION, gServerInfo.m_ServerName, gObjTotalUser, GAMESERVER_CLIENT, "https://MU-Noria.Com");
	}
...
void CServerDisplayer::PaintEventTime() // OK
{
	...
	//-----------------------------------------------------------------
	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "Sinh Ton: ");

	if (this->EventRoyalBattleGround == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->EventRoyalBattleGround == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds = this->EventRoyalBattleGround;
		hours = totalseconds / 3600;
		minutes = (totalseconds / 60) % 60;
		seconds = totalseconds % 60;
	...
	else if (this->EventRoyalBattleGround < 300)
	{
		SetTextColor(hdc, RGB(0, 190, 0));
	}
	else
	...
	//--
	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "SkyEvent: ");

	if (this->EventSkyEvent == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->EventSkyEvent == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds = this->EventSkyEvent;
		hours = totalseconds / 3600;
		minutes = (totalseconds / 60) % 60;
		seconds = totalseconds % 60;
		wsprintf(text2, "%02d:%02d:%02d", hours, minutes, seconds);
	...
	TextOut(hdc, posX1, rect.top + 2 + 180, text1, strlen(text1));
	if (this->EventSkyEvent == -1)
	{
		SetTextColor(hdc, RGB(255, 0, 0));
	}
	...
	else if (this->EventSkyEvent < 300)
	{
		SetTextColor(hdc, RGB(0, 190, 0));
	}
	else
	...
	TextOut(hdc, posX2, rect.top + 2 + 180, text2, strlen(text2));
#endif

	//============================================================================
	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "CTC MINI: ");

	if (this->EventCTCMini == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->EventCTCMini == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds = this->EventCTCMini;
		hours = totalseconds / 3600;
		minutes = (totalseconds / 60) % 60;
		seconds = totalseconds % 60;
		wsprintf(text2, "%02d:%02d:%02d", hours, minutes, seconds);
	...
	TextOut(hdc, posX1, rect.top + 2 + 195, text1, strlen(text1));
	if (this->EventCTCMini == -1)
	{
		SetTextColor(hdc, RGB(255, 0, 0));
	}
	...
	TextOut(hdc, posX2, rect.top + 2 + 195, text2, strlen(text2));
	//============================================================================

	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "EventGreatPK: ");

	if (this->EventCustomGreatPK[30] == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->EventCustomGreatPK[30] == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds = this->EventCustomGreatPK[30];;
		hours = totalseconds / 3600;
		minutes = (totalseconds / 60) % 60;
		seconds = totalseconds % 60;
		wsprintf(text2, "%02d:%02d:%02d", hours, minutes, seconds);
	...
	TextOut(hdc, posX1, rect.top + 2 + 225, text1, strlen(text1));
	if (this->EventCustomGreatPK[30] == -1)
	{
		SetTextColor(hdc, RGB(255, 0, 0));
	}
	...
	else if (this->EventCustomGreatPK[30] < 300)
	{
		SetTextColor(hdc, RGB(0, 190, 0));
	}
	else
	...
	TextOut(hdc, posX2, rect.top + 2 + 225, text2, strlen(text2));



#if(THANMA)
	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "Than Ma Chien: ");

	if (this->BEventThanMa == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->BEventThanMa == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds = this->BEventThanMa;
		hours = totalseconds / 3600;
		minutes = (totalseconds / 60) % 60;
		seconds = totalseconds % 60;
		wsprintf(text2, "%02d:%02d:%02d", hours, minutes, seconds);
	...
	TextOut(hdc, posX1, rect.top + 2 + 240, text1, strlen(text1));
	if (this->BEventThanMa == -1)
	{
		SetTextColor(hdc, RGB(255, 0, 0));
	}
	...
	TextOut(hdc, posX2, rect.top + 2 + 240, text2, strlen(text2));
#endif

#if BattleRoyaleEvent
	//============================================================================
	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "Battle Royal: ");

	if (this->EventBattleRoyale == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->EventBattleRoyale == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds = this->EventBattleRoyale;
		hours = totalseconds / 3600;
		minutes = (totalseconds / 60) % 60;
		seconds = totalseconds % 60;
		wsprintf(text2, "%02d:%02d:%02d", hours, minutes, seconds);
	...
	TextOut(hdc, posX1, rect.top + 2 + 255, text1, strlen(text1));
	if (this->EventBattleRoyale == -1)
	{
		SetTextColor(hdc, RGB(255, 0, 0));
	}
	...
	else if (this->EventBattleRoyale < 300)
	{
		SetTextColor(hdc, RGB(0, 190, 0));
	}
	else
	...
	TextOut(hdc, posX2, rect.top + 2 + 255, text2, strlen(text2));
#endif
...
void CServerDisplayer::PaintInvasionTime() // OK
{
	...
	for (int n = 0; n < 14; n++)
	{
		...
		SetTextColor(hdc, RGB(0, 102, 204));
		if (n == 0) { wsprintf(text1, "%d - %s", n, "Ngo Khi: "); }
		if (n == 1) { wsprintf(text1, "%d - %s", n, "Rong Do: "); }
		if (n == 2) { wsprintf(text1, "%d - %s", n, "Boss Vang: "); }
		if (n == 3) { wsprintf(text1, "%d - %s", n, "Phu Thuy: "); }
		if (n == 4) { wsprintf(text1, "%d - %s", n, "New Year: "); }
		if (n == 5) { wsprintf(text1, "%d - %s", n, "Tho Ngoc: "); }
		if (n == 6) { wsprintf(text1, "%d - %s", n, "Mua He: "); }
		if (n == 7) { wsprintf(text1, "%d - %s", n, "BoxFly: "); }
		if (n == 8) { wsprintf(text1, "%d - %s", n, "Medusa: "); }
		if (n == 9) { wsprintf(text1, "%d - %s", n, "BraveKing: "); }
		if (n == 10){ wsprintf(text1, "%d - %s", n, "Sieu Boss: "); }
		if (n == 11){ wsprintf(text1, "%d - %s", n, "Boss Class: "); }
		if (n == 12){ wsprintf(text1, "%d - %s", n, "Boss Custom"); }
		if (n == 13){ wsprintf(text1, "%d - %s", n, "Kho Bau: "); }
...
void CServerDisplayer::LogTextPaintConnect() // OK
{
	...
	for (int n = 0; n < MAX_LOGCONNECT_TEXT_LINE; n++)
	{
		...
		if (nn > 1)
		{
			TextOutW(hdc, rect.left + 10, (rect.top + 5 + (line * 15)), text_unicode, nn);
			line--;
		}
...
//	int OldBkMode = SetBkMode(hdc, TRANSPARENT);
//
//	HFONT MyFont = (HFONT)SelectObject(hdc,this->m_font3);
//
//	FillRect(hdc,&rect,this->m_brush[4]);
//
//	SetTextColor(hdc,RGB(0, 173, 181));
...
//		WCHAR text_unicode[100];
//
//		int nn = MultiByteToWideChar(CP_UTF8,0,this->m_logGlobal[count].text,size,text_unicode,100);
//
//		if(nn > 1)
//		{
//			TextOutW(hdc,rect.left+10,(rect.top+5+(line*15)),text_unicode, nn);
