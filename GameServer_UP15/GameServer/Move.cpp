...
void CMove::Move(LPOBJ lpObj, int index) // OK
{
	...
	if (gGate.GetGateMap(MoveInfo.Gate) == MAP_DUNES && gCustomGreatPK.CheckEnterEnabled(lpObj, MoveInfo.Gate) == 0)
	{
		return;
	}
	if (gGate.GetGateMap(MoveInfo.Gate) == MAP_DAU_TRUONG && gCustomArena.CheckEnterEnabled(lpObj, MoveInfo.Gate) == 0)
	{
		return;
	}
