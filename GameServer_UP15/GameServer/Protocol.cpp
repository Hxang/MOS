...
void GCPostItemProc(PMSG_POST_ITEM_RECV* lpMsg, int aIndex) //ok  
{
	...
	else
	{
		...
		tmpservercode += 1;
		char szTemp[256];
		wsprintf(szTemp, "[Sub-%d] %s", tmpservercode, lpMsg->chatmsg);
#endif
...
void ProtocolCore(BYTE head, BYTE* lpMsg, int size, int aIndex, int encrypt, int serial) // OK
{
	...
	case 0x42:
		...
		//gParty.CGPartyListRecv2(aIndex);
		break;
	case 0x43:
...
#if(SLOTPET)
		case 0x0A:
			g_pNewMyInventory.CGItemSellRecv((PMSG_EQUIP_ITEM_SELL_RECV*)lpMsg, aIndex);
			break;
#endif
	...
	case 0xF3:
...
#if(CTCMINI == 1)
		case 0x39: //Xu Ly Packet CTC mini
			gCTCMini.CGPacketCTCMini((CTCMINI_CGPACKET*)lpMsg, aIndex);
			break;
#endif
...
#if(RANKINGGOC == 1)
		case 0xE6:
			gRanking.GCReqRanking(aIndex, (PMSG_CUSTOM_RANKING_RECV*)lpMsg);
			break;
		case 0xE7:
			gRanking.GCReqRankingCount(aIndex, (PMSG_CUSTOM_RANKING_COUNT_RECV*)lpMsg);
			break;
#endif
		...
		case 0x27:
		{
			gCustomMoveEvent.MOVE_EVENT(lpObj, (MOVE_EVENT_REQ*)lpMsg, aIndex);
		}
		break;
...
#if(THONG_TIN_BOSS)
		case 0x02:
			gActiveInvasions.send_list_to_client(aIndex);
			break;
#endif
		}
		break;
	...
	case 0xF8:
		...
		case 0x4C:
			GuildUpgrade.RecvRequest((PMSG_GUILD_UPGRADE_REQUEST_RECV*)lpMsg, aIndex);
			break;
#endif
	...
	case 0xFA:
		...
		case 0x15:
		{
					 gCBCongHuong.CH_ClientSetItemGS(aIndex, (PMSG_MOVEITEM*)lpMsg);
		}
			break;
		case 0x16: //Rollback Item
		{
			...
			break;
		case 0x17: //Mix Cong Huong
		{
					   gCBCongHuong.CH_MixItemGS(aIndex, (XULY_CGPACKET1*)lpMsg);
		}
			break;
#endif
...
#if(JEWELBANKVER2)
		case 0x20:
			gBCustomItemBank.ClientSendRutOrGui((REQUEST_RUTGUIBANK*)lpMsg, aIndex);
			break;
#endif
		}
		break;
	...
	case 0xFC:
...
#if(DANHHIEU_NEW == 1)
		case 0x03:
			gDanhHieu.BUY_DANH_HIEU(lpObj, (BUY_DANH_HIEU_REQ*)lpMsg);
			break;
#endif

#if(QUANHAM_NEW == 1)
		case 0x04:
			gQuanHam.BUY_QUAN_HAM(lpObj, (BUY_QUAN_HAM_REQ*)lpMsg);
			break;
#endif

#if(TULUYEN_NEW == 1)
		case 0x05:
			gTuLuyen.BUY_TU_LUYEN(lpObj, (BUY_TU_LUYEN_REQ*)lpMsg);
			break;
#endif

		case 0x07:
			gLee.ReqHackRequest(lpObj, (REQUEST_HACK_REQ*)lpMsg);
			break;
		...
		case 0x24: // Làm Nhiệm Vụ
			gCommandManager.CommandChange(&gObj[aIndex], "", 0);
			break;
		...
		case 0x26: // Tẩy Điểm
			gCustomAttack.CommandCustomAttackOffline(lpObj, "offatack");
			break;

		case 0x27: // Tẩy Điểm
			gCommandManager.CommandReAdd(&gObj[aIndex], "", 0);
			break;

		case 0x28: // Tẩy Master
			gCommandManager.CommandReMaster(&gObj[aIndex], "", 0);
			break;
	...
	case 0xFD:
		...
		case 0x93:
			gBattleRoyale.CGOpenWindowNPC((XULY_CGPACKET2*)lpMsg, aIndex);
			break;
#endif
		}
		break;
	...
	case 0xFF:
		...
		case 0x14:
			...
			//==THan Ma Chien
		case 0x92:
			gThanMaChien.CGOpenWindowNPC((XULY_CGPACKET1*)lpMsg, aIndex);
			break;
#endif
	...
	case 0xD3:
		...
		case 0x6A:
		{
					 XULY_CGPACKET* Recv = (XULY_CGPACKET*)lpMsg;
					 if (Recv->ThaoTac != 111) return;
					 gSauChangeItem.SetStateInterface(lpObj->Index, 0);
		}
			break;
		...
		{
					   PMSG_MOVEITEM* Recv = (PMSG_MOVEITEM*)lpMsg;
					   gSauChangeItem.ProcItemSend(aIndex, Recv->Source);
		}
			break;
		...
		case 0x6C: //Back Item
		{
					   XULY_CGPACKET* Recv = (XULY_CGPACKET*)lpMsg;
					   if (Recv->ThaoTac == -1) return;
					   gSauChangeItem.BackItem(lpObj->Index, Recv->ThaoTac);
		}
			break;
#endif
		...
		case 0x9A: //Nhan Thuong
		{
			...
			break;
		case 0x9B: //Get List Nhan Thuong
		{
					   XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
					   if (!mlpMsg->ThaoTac) { return; }
					   //LogAdd(LOG_RED, "0x4B Get List %d", mlpMsg->ThaoTac);
		...
		break;
			...
			{
				XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
				int SkinIndex = mlpMsg->ThaoTac;
				if (!SkinIndex) { return; }
				gBCustomSkinModel.CGGetInfoSelectSkin(aIndex, SkinIndex);

			}
			break;
			//===MUa va thay doi skin
			case 0x12:
			{
				XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
				int SkinIndex = mlpMsg->ThaoTac;
				//if (!SkinIndex) { return; }
				gBCustomSkinModel.RecvActionSkin(aIndex, SkinIndex);
			}
			break;

			case 0x33:
			{
						 gCustomArena.SendBXHInfo(aIndex);
			}
				break;
			...
			case 0x34:
			{
				gCustomGreatPK.SendBXHInfo(aIndex);
			}
			break;
...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg, int aIndex) // OK
{
	...
	gLog.Output(LOG_CHAT, "[General] Account:[%s] Name:[%s] - (Message: %s)", lpObj->Account, lpObj->Name, lpMsg->message);
...
void CGChatWhisperRecv(PMSG_CHAT_WHISPER_RECV* lpMsg, int aIndex) // OK
{
	...
	gLog.Output(LOG_CHAT, "[Whisper] Account:[%s] Name:[%s] - (Name:%s, Message: %s)", lpObj->Account, lpObj->Name, name, lpMsg->message);
...
void CGMainCheckRecv(PMSG_MAIN_CHECK_RECV* lpMsg, int aIndex) // OK
{
	...
	if (gServerInfo.m_MainChecksum != 0)
	{
		...
		{
			gLog.Output(LOG_HACK, " Account:[%s] Name:[%s] Main CheckSum error", lpObj->Account, lpObj->Name);
		...
		{
			gLog.Output(LOG_HACK, " Account:[%s] Name:[%s] Invalid Main CheckSum", lpObj->Account, lpObj->Name);
			GCCloseClientSend(aIndex, 0);
			return;
		}
	}
...
void CGLiveClientRecv(PMSG_LIVE_CLIENT_RECV* lpMsg, int aIndex) // OK
{
	...
	{
		gLog.Output(LOG_HACK, "Account:[%s] Name:[%s] Client Latency Hack Detected [%d][%d]", lpObj->Account, lpObj->Name, (GetTickCount() - lpObj->ServerTickCount), (lpMsg->TickCount - lpObj->ClientTickCount));
		GCCloseClientSend(aIndex, 0);
		return;
	}
	...
	{
		gLog.Output(LOG_HACK, "Account:[%s] Name:[%s] Client Speed Hack Detected [%d][%d]", lpObj->Account, lpObj->Name, lpObj->PhysiSpeed, lpMsg->PhysiSpeed);
		GCCloseClientSend(aIndex, 0);
		return;
	}
	...
	{
		gLog.Output(LOG_HACK, "Account:[%s] Name:[%s] Client Speed Hack Detected [%d][%d]", lpObj->Account, lpObj->Name, lpObj->MagicSpeed, lpMsg->MagicSpeed);
		GCCloseClientSend(aIndex, 0);
		return;
	}
...
void GCAutoMove(AUTOMOVE_REQ* lpMsg, int aIndex) {
	...
	LogAdd(LOG_RED, "Đang Auto Di Chuyển %d , Name:%s", lpObj->IsAutoMoveRunning, lpObj->Name);
}
#endif
...
void CGMoveRecv(PMSG_MOVE_RECV* lpMsg, int aIndex) // OK
{
	...
	//{
	//	gMSGProtect.KiemTraDiChuyen(aIndex, lpObj->TX, lpObj->TY);
	//}
	////
}
...
void GCNewCharacterInfoSend(LPOBJ lpObj) // OK
{
	...
	//pMsg.WCoinC = lpObj->WCoinC; // G?i s? l??ng Wcoin ?ang có xu?ng Client
...
void ExtraCpanelSendInfo(int aIndex)
{
	...
	pInfo.MUOffHelperEnabled = g_OfflineMode.m_MUOffHelperEnabled[MAX_ACCOUNT_LEVEL];
...
void GCNewHealthBarSend(LPOBJ lpObj) // OK
{
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		if (lpTarget->Live == 0 || lpTarget->State != OBJECT_PLAYING || CC_MAP_RANGE(lpTarget->Map) != 0)
		{
			continue;
		}
...
void GetInfoClient(LPOBJ lpObj)
{
	PMSG_CLIENT_UPD pMsg;
	pMsg.h.set(0xFB, 200, sizeof(pMsg)); // Gửi Lên Client Protocol.cpp
	pMsg.DanhHieu = lpObj->rDanhHieu;
	pMsg.QuanHam = lpObj->rQuanHam;
	pMsg.TuLuyen = lpObj->rTuLuyen;
	pMsg.PartyNumberD = gParty.m_PartyInfo[lpObj->PartyNumber].Count;
...
//=====================================================================
#if(TEAMVSTEAM == 1)
void TvTEventEx(LPOBJ lpObj)
{
	// ----
	PMSG_TVTEVENT_UPD pMsg;
	pMsg.h.set(0xFB, 0x59, sizeof(pMsg)); // Gửi Lên Client Protocol.cpp

	TVT_EVENT_USER* lpUserA = gTvTEvent.GetUser(lpObj->Index);
	...
	TVT_EVENT_USER* lpUserB = gTvTEvent.GetUser(lpObj->Index);
	...
	pMsg.ScoreRed = gTvTEvent.ScoreRed;
	pMsg.ScoreBlue = gTvTEvent.ScoreBlue;
	pMsg.KillCount = lpUserA->Kills;
	pMsg.DieCount = lpUserB->Deaths;
	pMsg.TotalPlayer = gTvTEvent.TotalPlayer;
