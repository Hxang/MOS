...
{	
	...
	//MessageBox(0,"Phien Ban SERVER HieuDaiCa Share","Lien He 0962895867",MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
...
void CServerInfo::ReadCustomInfo() // OK
{
	...
	gCustomArena.ReadCustomArenaInfo("GameServerInfo",".\\Data\\GameServerInfo - Custom.ini");
	...
	gCustomTop.Load(gPath.GetFullPath("Custom\\CustomTop.xml"));
...
#if(RANKINGGOC == 1)
	gRanking.Load(gPath.GetFullPath("Custom\\CustomRanking.txt"));
#endif
...
//---------- cosas de pet
	...
	gCustomStartItem.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomStartItem\\CustomStartItem.txt"));

#if(BUFF_VUKHI_ICON)
	gCustomStartItemDame.LoadItemDame(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomStartItem\\CustomStartItemDame.txt"));
#endif
...
#if(OFFLINE_MODE_NEW)
	gOfflineMode.LoadSkillsInfo(gPath.GetFullPath("Custom\\DevHieuDaiCa\\OfflineMode\\OfflineModeBuffs.xml"));
	gOfflineMode.Load("OfflineMode", "..\\Data\\Custom\\DevHieuDaiCa\\OfflineMode\\OfflineMode.ini");
#endif
	...
	gThuongDanhBoss.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomRanking\\ThuongBoss.txt"));

#if(B_HON_HOAN)	
		gBHonHoan.LoadConfig(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomRanking\\CustomHonHoan.xml"));
#endif
#if(FLAG_SKIN)
		ThueFlag.Read(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomRanking\\CustomThueFlag.txt"));
#endif

#if (RESETCHANGE == 1)
		gResetChange.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\ResetChange.txt"));
#endif

#if (GHRS_TOP1_NEW == 1)
		gCustomGHRS.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomGHRS.ini"));
#endif

#if (CUSTOM_GUILD_UPGRADE == TRUE)
		GuildUpgrade.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\GuildUpgrade.txt"));
#endif

#if(VONGQUAY_NEW)
		gCustomVongQuay.LoadFileXML(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomVongQuay.xml"));
#endif

#if (CUSTOM_DUNGLUYENITEM)
		gBDungLuyenItem.LoadConfig(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomDungLuyenItem.xml"));
#endif
#if (SAUDOIITEM)
		gSauChangeItem.LoadConfig(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomDoiItem.xml"));
#endif
#if (CUSTOM_HARMONY)
		gCustomHarmony.LoadConfig(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomHarmony.xml"));
#endif
#if(B_MOCNAP)
		gCBMocNap.LoadFileXML(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomMocNap.xml"));
#endif
#if (CongHuongV2)
		gCBCongHuong.CH_LoadConfigXML(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomCongHuong.xml"));
#endif
#if(JEWELBANKVER2)
		gBCustomItemBank.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomItemBank.xml"));
#endif

	gHuyHieu.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CuongHoa\\CuongHoaSetItem.txt"));

	gBCustomSkinModel.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\CustomSkinModel.xml"));

	gShopPointEx.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\ShopPoint.txt"));

	gCustomGreatPK.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\Event\\EventGreatPK.xml"));

	gCustomArena.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\Event\\EventDauTruong.xml"));

	gPremiumDrop.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\PremiumDrop.xml"));

	gZenDrop.Load(gPath.GetFullPath("Custom\\DevHieuDaiCa\\ZenDrop.xml"));


	LogAdd(LOG_BLUE,"[ServerInfo] Custom Loaded Thành Công");
...
#if(TEAMVSTEAM == 1)
	gTvTEvent.Load(gPath.GetFullPath("Event\\TvTEvent\\TvTEvent.dat"));
#endif
...
#endif
#if BattleRoyaleEvent
	gBattleRoyale.Load(gPath.GetFullPath("Event\\BattleRoyale.xml"));
#endif
...
void CServerInfo::ReadHackInfo() // OK
{
...
#if(ANTIHACK_NEW)
	gBAntihack.LoadFileXML(gPath.GetFullPath("Hack\\AntiHackGS.xml"));
#endif
	...
	LogAdd(LOG_BLUE,"[ServerInfo] Hack Loaded Thành Công");
}
...
void CServerInfo::ReadItemInfo() // OK
{
	...
	LogAdd(LOG_BLUE,"[ServerInfo] Item Loaded Thành Công");
}
...
{
#if(SKY_EVENT == 1)
	SkyEvent.LoadData(gPath.GetFullPath("Event\\SkyEvent\\Config.xml"));
	SkyEvent.ReadMonsters(gPath.GetFullPath("Event\\SkyEvent\\Monster.dat"));
	LogAdd(LOG_BLACK, "[ServerInfo] SkyEvent Event Loaded Thành Công");
#endif
}
...
void CServerInfo::ReadMoveInfo() // OK
{
	...
	gRespawnLocation.Load(gPath.GetFullPath("Move\\RespawnLocation.xml"));
...
void CServerInfo::ReadUtilInfo() // OK
{
	...
	LogAdd(LOG_BLUE,"[ServerInfo] Util Loaded Thành Công");
}
...
void CServerInfo::ReloadBotInfo() // OK
{
...
#if(BOTRADE==1)
	BotTrader.Read(gPath.GetFullPath("Custom\\BotSystem\\BotTrader.txt"));
	BotTrader.MakeBot();
#endif
	//
#if(DUCTUONG)
	...
	s_FakeOnline.LoadFakeData(".\\AutoTrain.xml");
		...
		LogAdd(LOG_BLUE,"[ReadBotInfo] BotSystem Loaded Thành Công");
...
void CServerInfo::ReadStartupInfo(char* section,char* path) // OK
{
	...
	this->m_WriteResetLog = GetPrivateProfileInt(section, "WriteResetLog", 0, path);

	this->m_WriteTienTeLog = GetPrivateProfileInt(section, "WriteTienTeLog", 0, path);

	this->m_WriteKetNoiLog = GetPrivateProfileInt(section, "WriteKetNoiLog", 0, path);

	this->m_WriteThuMuaExcLog = GetPrivateProfileInt(section, "WriteThuMuaExcLog", 0, path);
...
void CServerInfo::ReadCharacterInfo(char* section,char* path) // OK
{
	...
	this->GioiHanLoaiBoPhongThu = GetPrivateProfileInt(section, "GioiHanLoaiBoPhongThu", 0, path);
...
void CServerInfo::ReadCommandInfo(char* section,char* path) // OK
{
	...
	this->m_CommandResetAddPoinLeadership[0] = GetPrivateProfileInt(section,"CommandResetAddPoinLeadership_AL0",0,path);

	this->m_CommandResetAddPoinLeadership[1] = GetPrivateProfileInt(section,"CommandResetAddPoinLeadership_AL1",0,path);

	this->m_CommandResetAddPoinLeadership[2] = GetPrivateProfileInt(section,"CommandResetAddPoinLeadership_AL2",0,path);

	this->m_CommandResetAddPoinLeadership[3] = GetPrivateProfileInt(section,"CommandResetAddPoinLeadership_AL3",0,path);
...
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	this->m_SharkGuardCRC = GetPrivateProfileInt(section, "SharkGuardCRC", 0, path);
	...
	this->m_VerifyCRC = GetPrivateProfileInt(section, "VerifyDllCRC", 0, path);
...
#if (RESETCHANGE == 1)
	this->PointDoiReset = GetPrivateProfileInt(section, "PointDoiReset", 0, path);
#endif
	...
	this->m_PotionDelay = GetPrivateProfileInt(section, "PotionDelay", 0, path);
	...
	this->m_memberOnline = GetPrivateProfileInt(section, "ShowMembOnline", 0, path);
...
void CServerInfo::ReadCustomInfo(char* section,char* path) // OK
{
	...
	this->m_InventoryWearGetGift = GetPrivateProfileInt(section, "InventoryWearGetGift", 0, path);
	this->m_InventoryMainGetGift = GetPrivateProfileInt(section, "InventoryMainGetGift", 0, path);
	this->m_NumBerGetGift = GetPrivateProfileInt(section, "NumBerGetGift", 0, path);
	...
	this->m_ThoiGianMuaHang = GetPrivateProfileInt(section, "ThoiGianMuaHang", 1000, path);
	...
	this->NewsWindowOnLogin = GetPrivateProfileInt(section, "NewsWindowOnLogin", 0, path);
...
#if (SKY_EVENT == 1)	
	this->SkyEnabled = GetPrivateProfileInt(section, "SkyEventEnabled", 0, path);
#endif
...
void CServerInfo::ReadHackInfo(char* section,char* path) // OK
{
	...
	this->m_XdameHackSkillEnable = GetPrivateProfileInt(section, "XdameEnable", 0, path);

	this->m_XdameHackSkillPenalty = GetPrivateProfileInt(section, "XdameDetectionTime ", 0, path);

	this->m_XdameHackSkillDebug = GetPrivateProfileInt(section, "XdameTest", 0, path);

	this->m_XdameHackSkillType = GetPrivateProfileInt(section, "XdameType", 0, path);

	this->m_XdameHackSkillDialog = GetPrivateProfileInt(section, "XdameNotice", 0, path);

	this->m_timedongbang = GetPrivateProfileInt(section, "XdameBlock", 0, path);
