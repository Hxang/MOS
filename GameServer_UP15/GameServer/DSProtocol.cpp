...
void DataServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0xD9:
			...
			{
			case 0x01:
				gBCustomSkinModel.RecvGDGetListBuy((CBCUSTOM_LOAD_COUNT*)lpMsg);
				break;
#if (JEWELBANKVER2)
			case 0x21://=== Custom Item Bank Nhan KQ TU DS
				gBCustomItemBank.DSSetInfoUserBankItem((GSSENDDS_COUNT_ITEMBANK*)lpMsg);
				break;
...
#if(RANKINGGOC == 1)
		case 0xF4:
			gRanking.GDCustomRankingRecv((SDHP_CUSTOM_RANKING_RECV*)lpMsg);
			break;
#endif
		case 0xF5:
...
#if (GHRS_TOP1_NEW == 1)
		case 0xF6:
			gCustomGHRS.GDCustomGHRSRecv((SDHP_CUSTOM_GHRS_RECV*)lpMsg);
			break;
#endif
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
...
#if(MEMBER_ONLINE)
	//post mem online
	if (gServerInfo.m_memberOnline == 1){
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(956), gObjTotalUser, gServerInfo.m_ServerMaxUserNumber);
	...
	//gBuffInfoNew.BuffIDItem(lpObj); //dang lam
...
#if (RESETCHANGE == 1)
	gResetChange.EnvioResetChange(lpObj->Index);
#endif

	//===Get Info Skin
	gBCustomSkinModel.GDGetListBuy(lpObj->Index); //Lay Danh Sach Da Mua
	...
	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(990), gCustomGHRS.maxResets);
	...
	//CItem* lpItem = &lpObj->Inventory[lpMsg->TargetFlag];
	CItem* lpItem = &lpObj->Inventory[0];
	CItem* lpItem2 = &lpObj->Inventory[1];
	//tesst  load doi xiu 
	//CustomStartItemDame_INFO* lpInfo = &gCustomStartItemDame.m_CustomStartItemDameInfo[lpObj->Class]; build test 
...
#if (JEWELBANKVER2)
	//===Get KQ Bank
	gBCustomItemBank.GSSendDSGetData(lpObj->Index);
#endif	
	...
	if (info != 0 && lpObj->Socket == INVALID_SOCKET)
	{
		...
		if (info->TuDongReset == 1) //20-03-2022 Fix
		{
			lpObj->AutoResetEnable = 1;
			//20-03-2022  Sửa lại lưu DataServer
			gCustomAttack.DGCustomAttackResumeSaveSend(lpObj->Index);
		}

		LogAdd(LOG_RED, "[FakeOnline][Obj][%d] (%s) OK", lpObj->Index, lpObj->Name);
	}
#endif
...
void DGGlobalPostRecv(SDHP_GLOBAL_POST_RECV* lpMsg) // OK
{
	...
	switch(lpMsg->type)
	{
		...
		case 1:
			...
			break;
		case 2:
			PostMessage3(lpMsg->name,gMessage.GetMessage(25),lpMsg->message);
			break;
		case 3:
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
	...
	//==CTC Save ve DB
	pMsg.CTCTime = lpObj->CTCTime;
	pMsg.CTCRegDay = lpObj->CTCRegDay;


	pMsg.mUserSkinPick = lpObj->UserSkinPick;

#if(B_HON_HOAN)
	pMsg.CapDoHonHoan = lpObj->CapDoHonHoan;
#endif
#if(FLAG_SKIN)
