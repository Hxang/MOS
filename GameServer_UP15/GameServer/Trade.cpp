...
void CTrade::ResetTrade(int aIndex) // OK
{
	...
	g_pNewMyInventory.GCPetItemListSend(aIndex);  
...
void CTrade::CGTradeRequestRecv(PMSG_TRADE_REQUEST_RECV* lpMsg, int aIndex) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(873));
		return;
	}
	...
	if (lpObj->CB_Status != 0 || lpTarget->CB_Status != 0)
	{
		return;
	}
...
void CTrade::CGTradeOkButtonRecv(PMSG_TRADE_OK_BUTTON_RECV* lpMsg, int aIndex) // OK
{
	...
	g_pNewMyInventory.GCPetItemListSend(bIndex);
