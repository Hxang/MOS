	...
	for (int n = 0; n < MAX_THANMACHIEN_USER; n++)
	{
		if (OBJECT_RANGE(this->User[n].Index) != 0)
		{
			...
			if (this->User[n].Team == TeamReward)
			{
				...
				{
					GDSetCoinSend(lpObj->Index, this->Coin1, this->Coin2, this->Coin3, "Thần Ma");
				}
...
void CThanMaChien::UserDieProc(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	{
		GDSetCoinSend(lpUserB->Index, this->BonusKill[0], this->BonusKill[1], this->BonusKill[2], "Thưởng Coin Thần Ma");
	}
	//== Tinh Toan Rank
	this->CalcUserRank();
