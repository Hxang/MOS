...
void gBMocNap::NhanThuongMocNap(int aIndex, int MocNap)
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, this->GetMessage(2), MocNap - 1); //Nhan MOc Nho hon truoc
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, this->GetMessage(3), BNumberFormat(it->second.GiaTriNap)); //Yeu cau gia tri nap
		return;
	}
	if (it->second.IndexMocNap <= lpObj->NhanMocNap)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, this->GetMessage(4)); //da nhan roi
		return;
	}
	//===Kiem tra thung do
