...
void OfflineMode::Attack(int aIndex)
{
	...
	if (lpObj->SkillBasicID < 1 && lpObj->BuffOn == 0)
	{
		return;
	}
...
void OfflineMode::EmptyMuOffHelper(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];

	/*+172*/lpObj->HuntingRange = 0;
	/*+176*/lpObj->RecoveryPotionOn = 0;
	/*+177*/lpObj->RecoveryPotionPercent = 0;
	/*+178*/lpObj->RecoveryHealOn = 0; //-> CLASS_ELF
	/*+179*/lpObj->RecoveryHealPercent = 0; //-> CLASS_ELF
	/*+180*/lpObj->RecoveryDrainOn = 0; //-> CLASS_SUMMONER
	/*+181*/lpObj->RecoveryDrainPercent = 0; //-> CLASS_SUMMONER
	//--
	/*+182*/lpObj->DistanceLongOn = 0;
	/*+183*/lpObj->DistanceReturnOn = 0;
	/*+184*/lpObj->DistanceMin = 0;
	/*+188*/lpObj->SkillBasicID = 0;

	/*+240*/lpObj->ComboOn = 0; //-> CLASS_KNIGHT
	/*+241*/lpObj->PartyModeOn = 0;
	/*+242*/lpObj->PartyModeHealOn = 0; //-> CLASS_ELF
	/*+243*/lpObj->PartyModeHealPercent = 0; //-> CLASS_ELF
	/*+244*/lpObj->PartyModeBuffOn = 0;

	/*+260*/lpObj->BuffOn = 0;
	/*+264*/lpObj->BuffSkill[0] = 0;
	/*+264*/lpObj->BuffSkill[1] = 0;
	/*+264*/lpObj->BuffSkill[2] = 0;
	/*+288*/lpObj->ObtainRange = 0;
	/*+292*/lpObj->ObtainRepairOn = 0;
	/*+293*/lpObj->ObtainPickNear = 0;
	/*+294*/lpObj->ObtainPickSelected = 0;
	/*+295*/lpObj->ObtainPickJewels = 0;
	/*+296*/lpObj->ObtainPickAncient = 0;
	/*+297*/lpObj->ObtainPickMoney = 0;
	/*+298*/lpObj->ObtainPickExcellent = 0;
	/*+298*/lpObj->ObtainPickExcellent = 0;
	/*+299*/lpObj->ObtainPickExtra = 0;
}
...
void OfflineMode::RenderAutoPote(int aIndex)	//-- OK
{
	...
	//-- RECOVERY DRAIN SUMMONER
	if (lpObj->RecoveryDrainOn != 0)
	{
		CSkill* RenderSkillDrainLife;

		RenderSkillDrainLife = gSkillManager.GetSkill(lpObj, SKILL_DRAIN_LIFE);

		if (RenderSkillDrainLife != 0)
		{
			if (lpObj->Life < ((lpObj->MaxLife * lpObj->RecoveryDrainPercent) / 100))
			{
				if (gEffectManager.CheckEffect(lpObj, gSkillManager.GetSkillEffect(RenderSkillDrainLife->m_index)) == 0)
				{
					gSkillManager.UseAttackSkill(lpObj->Index, lpObj->Index, RenderSkillDrainLife);
				}
			}
		}
	}
...
int OfflineMode::PickUP(int aIndex)
{
	...
	for (int n = 0; n < MAX_MAP_ITEM; n++)
	{
		...
		if (lpMapItem->IsItem() == TRUE  && lpMapItem->m_Give == false && lpMapItem->m_Live == true)
		{
			...
			/*if(lpObj->ObtainPickNear == 1)
			{
			PickItem = true;
			AnimationMove(lpObj->Index, lpMapItem->m_X, lpMapItem->m_Y);
			}
			else */if (lpObj->ObtainPickSelected == 1)
			{
