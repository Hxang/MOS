...
bool CSkillManager::SkillChangeUse(int aIndex) // OK
{
	...
	if (change == -1)
	{
		change = gBCustomSkinModel.GetCBSkinChange(aIndex);
	}
	...
	else
	{
		...
		for (int n = 0; n < MAX_PARTY_USER; n++)
		{
			...
			if (lpObj->Map == gObj[lpParty->Index[n]].Map && gObjCalcDistance(lpObj, &gObj[lpParty->Index[n]]) < MAX_PARTY_DISTANCE)
			{
				...
				{
					GCLifeSend(index, 0xFF, (int)lpTarget->Life, lpTarget->Shield);
				}
...
bool CSkillManager::SkillGreaterDamage(int aIndex, int bIndex, CSkill* lpSkill) // OK
{
	...
	{
		PARTY_INFO* lpParty = &gParty.m_PartyInfo[lpObj->PartyNumber];
		const int Health = value; //fix kuti
		for (int n = 0; n < MAX_PARTY_USER; n++)
		{
			...
			if (lpObj->Map == gObj[lpParty->Index[n]].Map && gObjCalcDistance(lpObj, &gObj[lpParty->Index[n]]) < MAX_PARTY_DISTANCE)
			{
				...
				//fix kuti
				value = (Health*((lpTarget->Type == OBJECT_USER) ? gServerInfo.m_GreaterDamageRate[lpTarget->Class] : 100)) / 100;
...
void CSkillManager::CGMultiSkillAttackRecv(PMSG_MULTI_SKILL_ATTACK_RECV* lpMsg, int aIndex, bool type) // OK
{
	...
	if (gHackXDameCheck.CheckXDameHack(lpObj, skill) == 1)
	{
		return;
	}
	if (lpObj->Type == OBJECT_USER)
