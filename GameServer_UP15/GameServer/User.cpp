...
//	lpObj->AttackCustomOfflineMoneyDelay = 0;
	...
	/*NewAnti*/
	lpObj->MSGProtect_X = 0;
	lpObj->MSGProtect_Y = 0;
	lpObj->MSGProtect_MAP = 0;
	lpObj->MSGProtectMove_Time = 0;
	lpObj->MSGProtectAnimationTime = 0;
...
#if (SAUDOIITEM)
	lpObj->SAU_StatusMix = 0;
	lpObj->SAU_IndexMix = -1;
...
short gObjDel(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		if(lpObj->Account[0] != 0){LogAddConnect(LOG_BLUE,"[Obj] [%d] Thoát Game Account:(%s)",aIndex,lpObj->Account);}

		LogAddConnect(LOG_BLUE, "[Obj] [%d] Ip Thoát Game (%s)", aIndex, lpObj->IpAddr);
...
bool gObjFixInventoryPointer(int aIndex) // OK
{
	...
	{
#if BattleRoyaleEvent
		if (lpObj->Transaction == 1 || lpObj->CB_Status == 1)
#else
...
bool gObjInventoryTransaction(int aIndex) // OK
{
	...
	if (lpObj->Transaction == 1)
	{
		...
		gObjUserKill(aIndex);
		gLog.Output(LOG_TRADE, "[Fix Inv.Ptr] [%s][%s] - Transaction == 1, Do not change Pointer", gObj[aIndex].Account, gObj[aIndex].Name);
		return 0;
	}
...
void gObjInterfaceCheckTime(LPOBJ lpObj) // OK
{
	...
	//if (lpObj->Interface.use == 0 && lpObj->Interface.type == 6)
	//{
...
void SendKillToUsers(int aIndex, int bIndex)
{
	//LogAdd(LOG_RED, "Skin Chua Mua");
	PSWMSG_KILL_SEND  pMsg = { 0 };
...
void gObjUserDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (gCustomGreatPK.UserDieProc(lpObj, lpTarget))
	{
		return;
	}
	if (gCustomArena.UserDieProc(lpObj, lpTarget))
	{
		return;
	}
	...
	if (lpTarget->Type == OBJECT_USER && lpObj->IsFakeOnline == true) // Fix Neu bi nguoi kill
	{
		lpObj->IsFakeRegen = false;
...
#endif
#if BattleRoyaleEvent
	if (gBattleRoyale.CheckPlayerTarget(lpObj))
	{
		gBattleRoyale.UserDieProc(lpObj, lpTarget);
	}
#endif
	...
	if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
		lpTarget->Kills++;
		lpObj->Deads++;
		SendKillToUsers(lpTarget->Index, lpObj->Index);
		...
		LogAdd(LOG_RED,"Account:[%s] Name:[%s] Giết  Account:[%s] Name:[%s]",lpTarget->Account,lpTarget->Name,lpObj->Account,lpObj->Name);


	}

	#if OLD_NOTICEPK
	if (lpObj->CB_Status == 0 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
	...
	{
		if (lpObj->PKLevel >= 6)//[FIX14] pk drop level
		{
			count = 24;
			...
			while(count-- != 0)
			{
				...
				if(lpObj->Inventory[number].IsItem()==1)
				{
						...
						{
							LogAdd(LOG_USER,"Account:[%s] Name:[%s] PK Drop, ItemName:%s",lpObj->Account,lpObj->Name,gItemManager.GetItemName(lpObj->Inventory[number].m_Index));
							dropresult = 1;
							break;
						}
				}
...
void gObjPlayerKiller(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
...
#if BattleRoyaleEvent
	if (gBattleRoyale.CheckPlayerTarget(lpObj) && gBattleRoyale.CheckPlayerTarget(lpTarget))
	{
		return;
	}
#endif
	...
	if (gCustomGreatPK.CheckPlayerJoined(lpObj, lpTarget) != 0)
	{
		return;
	}
	if (gCustomArena.CheckPlayerJoined(lpObj, lpTarget) != 0)
	{
		return;
	}
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if(lpObj->Type == OBJECT_USER)
			{
				...
				//	gParty.GCPartyLifeSend2(lpObj->PartyNumber);
		...
		if (lpObj->Connected > OBJECT_LOGGED &&	lpObj->Type == OBJECT_USER)
		{
			if (GetTickCount() - lpObj->AutoSaveTime > 600000)
			{
				...
				lpObj->AutoSaveTime = GetTickCount();
				//LogAdd(LOG_RED, "[GDCharacterInfoSaveSend] OK");
#if (JEWELBANKVER2)
				gBCustomItemBank.GSSendDSSaveDataUser(lpObj->Index);
#endif
			}

			if (lpObj->CheckSumTime > 0 && GetTickCount() - lpObj->CheckSumTime > 60000 && lpObj->IsFakeOnline != true)
			{
				LogAdd(LOG_BLACK, "[%s][%s] CheckSumTime Lỗi!", lpObj->Account, lpObj->Name);
		...
		if (lpObj->Connected == OBJECT_CONNECTED || lpObj->Connected == OBJECT_LOGGED || lpObj->Connected == OBJECT_ONLINE && lpObj->IsFakeOnline != true)
		{
			if(lpObj->Type == OBJECT_USER)
			{
				if(lpObj->Connected >= OBJECT_LOGGED)
				{
					if(GetTickCount() - lpObj->ConnectTickCount > 60000)
					{
						...
						if (lpObj->m_OfflineMode == 1) //Fake Online
						{
							lpObj->ConnectTickCount = GetTickCount();
							continue;
						}
						...
						CloseClient(n);
						LogAdd(LOG_RED, "Ngắt Kết Nối Đang Nhập Game [%d][%s][%s][%s]", lpObj->Index, lpObj->Account, lpObj->Name, lpObj->IpAddr);
					}
				}
				else
				{
					if(GetTickCount() - lpObj->ConnectTickCount > 90000) //Thời Gian Login Đăng Nhập
					{
						CloseClient(n);
						LogAdd(LOG_RED, "Hết Time Ngắt Kết Nối Đăng Nhập [%d][%s][%s][%s]", lpObj->Index, lpObj->Account, lpObj->Name, lpObj->IpAddr);
					}
				}
			}
		}
