...
void CWarehouse::DGWarehouseItemRecv(SDHP_WAREHOUSE_ITEM_RECV* lpMsg) // OK
{
	...
	{
		LogAdd(LOG_RED, "[DGWarehouseItemRecv] Tài khoản không hợp lệ [%d] (%s)", lpMsg->index, lpMsg->account);
...
void CWarehouse::DGWarehouseFreeRecv(SDHP_WAREHOUSE_FREE_RECV* lpMsg) // OK
{
	if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0)
	{
		LogAdd(LOG_RED, "[DGWarehouseFreeRecv] Tài khoản không hợp lệ [%d](%s)", lpMsg->index, lpMsg->account);
		CloseClient(lpMsg->index);
		return;
	}
