...
void CFakeOnline::RestoreFakeOnline()
{
	...
	for (std::map<std::string, OFFEXP_DATA>::iterator it = this->m_Data.begin(); it != this->m_Data.end(); it++)
	{
		...
		if (aIndex >= 0)
		{
			...
			LogAdd(LOG_RED, "[FakeOnline]  [TK: %s NV: %s] Online", it->second.Account, it->second.Name);
		}
	}
}
	...
	if (info != 0 && lpObj->Socket == INVALID_SOCKET)
	{
		if (info->PostKhiDie == 1)
		{
			...
			else if (gServerInfo.m_CommandPostType == 5)
			{
				...
				tmpservercode += 1;
				char szTemp[256];
				wsprintf(szTemp, "[Sub-%d] %s", tmpservercode, gMessage.GetMessage(rand() % (this->IndexMsgMax - this->IndexMsgMin + 1) + this->IndexMsgMin));
...
void CFakeOnline::QuayLaiToaDoGoc(int aIndex)
{
	...
	if (info != 0 && lpObj->Socket == INVALID_SOCKET)
	{
		...
		if (GetTickCount() >= lpObj->m_OfflineTimeResetMove + 1000) // 2000
		{
			if ((PhamViDiTrain >= (lpObj->IsFakeMoveRange + 1) && !lpObj->IsFakeRegen) || gServerInfo.InSafeZone(lpObj->Index) == true)
			{
		...
		if (lpObj->IsFakeMoveRange != 0) {
			if (GetTickCount() >= lpObj->m_OfflineTimeResetMove + DelayRange && lpObj->IsFakeRegen) // 2000
			{
				int MoveRange = 3;
				//======Move Range======//
				...
				while (searchc-- != 0)
				{
					...
					if ((attr & 1) != 1 && (attr & 2) != 2 && (attr & 4) != 4 && (attr & 8) != 8 && GetTickCount() >= lpObj->m_OfflineMoveDelay + DelayRange) // 1000
					{
	...
	{
		if (GetTickCount() >= lpObj->m_OfflineTimeResetMove + 10 + ((lpObj->DistanceMin * 60) * 5)) // 1000   60   10
		{
