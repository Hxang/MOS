...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if (lpObj->Interface.use != 0) //Check Dupe
	{
		return 1;
	}
	...
	if (gBattleRoyale.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
#endif
...
#if(SAUDOIITEM)
	if (gSauChangeItem.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
#endif
	...
	{
		GCChatTargetSend(lpObj, lpNpc->Index, gMessage.GetMessage(865));
		return 1;
	}
	//--
	switch(lpNpc->Class)
	{
		...
		case 450:
#if(SKY_EVENT == 1)
			if (SkyEvent.NPCEnabled == 1 && lpObj->Map == MAP_DEVIAS){
				SkyEvent.NpcTalk(lpNpc, lpObj);
			}
			else{
				if (SkyEvent.NPCEnabled == 0 && lpObj->Map == MAP_DEVIAS){
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "[Sky Event (Thiên Giới)] Sự Kiện Chưa Bắt Đầu");
				}
				else{
...
void CNpcTalk::NpcGuildMaster(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	if(lpObj->GuildNumber > 0)
	{
		if (gServerInfo.m_GuildWarehouseEnable == 1)
		{
			...
			{
				//gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Intentando abrir");
