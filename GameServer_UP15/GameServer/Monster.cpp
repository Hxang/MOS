...
{
	gThuongDanhBoss.Thuong(lpObj, lpTarget);
	...
	if (lpObj->Class == 561)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(207), lpTarget->Name);
	}
...
int gObjMonsterSearchEnemy(LPOBJ lpObj, BYTE objtype)
{
	...
	for (n = 0; n<MAX_VIEWPORT; n++)
	{
		...
		if (gDuel.GetDuelArenaBySpectator(gObj[tObjNum].Index) != 0){
			continue;
		}
#endif
#endif
