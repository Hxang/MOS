...
#if (JEWELBANKVER2)
	int GetSlotItemBank = gBCustomItemBank.CheckInfoListItemBank(lpItem->m_Index, lpItem->m_Level);
	if (GetSlotItemBank != -1)
	{
		if (gBCustomItemBank.CheckItemBankAutoPick(aIndex, lpItem->m_Index, lpItem->m_Level))
		{
			if (gBCustomItemBank.CongTruBank(aIndex, gBCustomItemBank.mListItemBank[GetSlotItemBank].ItemIndex, gBCustomItemBank.mListItemBank[GetSlotItemBank].ItemLevel, +(1)))
			{
				lpItem->Clear();
				pMsg.result = -1;
	...
	if (lpMsg->SourceFlag == 0 && lpMsg->TargetFlag == 0) // Inventory -> Inventory
	{
		...
		if ((pMsg.result = this->MoveItemToInventoryFromInventory(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			...
			for (std::vector<CUSTOM_START_ITEM_DAME_INFO>::iterator it = lpInfo->ItemDameStart.begin(); it != lpInfo->ItemDameStart.end(); it++)
			{
				...
				else if (lpItem->m_Index == GET_ITEM(it->ItemType, it->ItemIndex) || lpItem1->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					if ((lpItem->m_Level >= it->LevelMin) && (lpItem->m_Level <= it->LevelMax) || (lpItem1->m_Level >= it->LevelMin) && (lpItem1->m_Level <= it->LevelMax))
					{
						if (lpItem->m_NewOption >= it->ItemOption || lpItem1->m_NewOption >= it->ItemOption)
						{

							gEffectManager.AddEffect(lpObj, (gCustomStartItemDame.CheckSkillEffectDame(it->EffectBuff) == true) ? 0 : 1, it->EffectBuff, (it->TypeBuff == 0) ? (int)(time(0) + it->TimeBuff) : it->TimeBuff, it->Power1Buff, it->Power2Buff, 0, 0);
			...
			for (std::vector<CustomStartSetItemDame_Data>::iterator it = gCustomStartSetItemDame.itemdata.begin(); it != gCustomStartSetItemDame.itemdata.end(); it++)
			{
				...
				if (lpItemHelm->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), 0, 0, 0, 0);
...
Next:
	...
	else if ((lpItem->m_Index >= GET_ITEM(14, 0) && lpItem->m_Index <= GET_ITEM(14, 6)) || (lpItem->m_Index >= GET_ITEM(14, 35) && lpItem->m_Index <= GET_ITEM(14, 40)) || lpItem->m_Index == GET_ITEM(14, 70) || lpItem->m_Index == GET_ITEM(14, 71) || lpItem->m_Index == GET_ITEM(14, 133))
	{
		...
		// ----
		if (Delay < (DWORD)gServerInfo.m_PotionDelay)
		{
			return;
		}
		else
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg, int aIndex, int ok) // OK
{
	...
	if (this->InventoryInsertItemStack(lpObj, &item) != 0)
	{
		if (item.Coin1 > 0 || item.Coin2 > 0 || item.Coin3 > 0)
		{
			GDSetCoinSend(lpObj->Index, -(item.Coin1*valuex), -(item.Coin2*valuex), -(item.Coin3*valuex), "Mua Item Ở Shop");
		}
		else
		{
...
void CItemManager::CGItemSellRecv(PMSG_ITEM_SELL_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpItem->Sell > 0)
	{
		if (lpItem->Coin1 > 0)
		{
			GDSetCoinSend(lpObj->Index, value, 0, 0, "Bán Item Vào Shop");
		}
		else if (lpItem->Coin2 > 0)
		{
			GDSetCoinSend(lpObj->Index, 0, value, 0, "Bán Item Vào Shop");
		}
		else if (lpItem->Coin3 > 0)
		{
			GDSetCoinSend(lpObj->Index, 0, 0, value, "Bán Item Vào Shop");
		}
		else
		{
...
BYTE CItemManager::MoveItemToEquipFromTrade(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag)
{
	if (TRADE_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
...
BYTE CItemManager::MoveItemToTradeFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag)
{
	if (EQUIP_INVENTORY_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
