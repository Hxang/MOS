...
bool CSocketManager::Start(WORD port) // OK
{
	...
	gLog.Output(LOG_CONNECT,"[SocketManager] Đã Khởi Động Port [%d] Thành Công",this->m_port);
	return 1;
}
...
bool CSocketManager::CreateListenSocket() // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] WSASocket() không thành công với lỗi: %d",WSAGetLastError());
		return 0;
	}
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] bind() không thành công với lỗi: %d",WSAGetLastError());
		return 0;
	}

	if(listen(this->m_listen,5) == SOCKET_ERROR)
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] listen() không thành công với lỗi: %d",WSAGetLastError());
		return 0;
	}
...
bool CSocketManager::CreateCompletionPort() // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] socket() không thành công với lỗi: %d",WSAGetLastError());
		return 0;
	}

	if((this->m_CompletionPort=CreateIoCompletionPort((HANDLE)socket,0,0,0)) == 0)
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] CreateIoCompletionPort() không thành công với lỗi: %d",GetLastError());
		closesocket(socket);
		return 0;
	}
...
bool CSocketManager::CreateAcceptThread() // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] CreateThread() không thành công với lỗi: %d",GetLastError());
		return 0;
	}

	if(SetThreadPriority(this->m_ServerAcceptThread,THREAD_PRIORITY_HIGHEST) == 0)
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] SetThreadPriority() không thành công với lỗi: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::CreateServerQueue() // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] CreateSemaphore() không thành công với lỗi: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while(true)
	{
		...
		{
			gLog.Output(LOG_CONNECT, "[SocketManager] Lỗi tiêu đề giao thức (Chỉ mục: %d, Tiêu đề: %x)", index, lpMsg[count]);
			return 0;
		}

		if(size < 3 || size > MAX_MAIN_PACKET_SIZE)
		{
			gLog.Output(LOG_CONNECT,"[SocketManager] Lỗi kích thước giao thức (Chỉ mục: %d, Tiêu đề: %x, Kích thước: %d, Đầu: %x)",index,header,size,head);
			return 0;
		}
...
bool CSocketManager::DataSend(int index,BYTE* lpMsg,int size) // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] Kích thước msg tối đa (Loại: 1, Chỉ mục: %d, Kích thước: %d)",index,size);
	...
	if(lpIoContext->IoSize > 0)
	{
		...
		{
			gLog.Output(LOG_CONNECT, "[SocketManager] Kích thước msg tối đa (Loại: 2, Chỉ mục: %d, Kích thước: %d)", index, (lpIoContext->IoSideBuffer.size + size));
			this->Disconnect(index);
			this->m_critical.unlock();
			return 0;
		}
	...
	if(WSASend(gObj[index].Socket,&lpIoContext->wsabuf,1,&SendSize,Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		...
		{
			gLog.Output(LOG_CONNECT,"[SocketManager] WSASend() không thành công với lỗi: %d",WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return 0;
		}
...
void CSocketManager::Disconnect(int index) // OK
{
	...
	{
		gLog.Output(LOG_CONNECT,"[SocketManager] closesocket() không thành công với lỗi: %d",WSAGetLastError());
		this->m_critical.unlock();
		return;
	}
...
void CSocketManager::OnRecv(int index,DWORD IoSize,IO_RECV_CONTEXT* lpIoContext) // OK
{
	...
	if(WSARecv(lpObj->Socket,&lpIoContext->wsabuf,1,&RecvSize,&Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSA_IO_PENDING)
		{
			gLog.Output(LOG_CONNECT,"[SocketManager] WSARecv() không thành công với lỗi: %d",WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return;
		}
...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		...
		if(socket == SOCKET_ERROR && WSAGetLastError() != WSAEWOULDBLOCK)
		{
			...
			gLog.Output(LOG_CONNECT,"[SocketManager] WSAAccept() không thành công với lỗi: %d",WSAGetLastError());
		...
		{
			gLog.Output(LOG_CONNECT,"[SocketManager] gObjAdd() không thành công với lỗi: %d",GetLastError());
			closesocket(socket);
			lpSocketManager->m_critical.unlock();
			continue;
		}
...
DWORD WINAPI CSocketManager::ServerWorkerThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		if(GetQueuedCompletionStatus(lpSocketManager->m_CompletionPort,&IoSize,&index,&lpOverlapped,INFINITE) == 0)
		{
			...
			{
				lpSocketManager->m_critical.lock();
				gLog.Output(LOG_CONNECT,"[SocketManager] GetQueuedCompletionStatus() không thành công với lỗi: %d",GetLastError());
				lpSocketManager->m_critical.unlock();
				return 0;
			}
		}
...
DWORD WINAPI CSocketManager::ServerQueueThread(CSocketManager* lpSocketManager) // OK
{
	while(true)
	{
		...
		{
			gLog.Output(LOG_CONNECT,"[SocketManager] WaitForSingleObject() không thành công với lỗi: %d",GetLastError());
			break;
		}
