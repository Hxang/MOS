...
#pragma region ATTACK_CHECK
	...
	if (RoyalBattleGround.CheckPrepareZone(lpTarget->Index))
	{
		return 0;
	}
...
#pragma region DAMAGE_CONFIG
	...
	if (lpObj->Type == OBJECT_USER)
	{
		...
		else
		{
			...
			//Fix DameRate
			float gCalcDameRate = (float)(gServerInfo.m_GeneralDamageRatePvM) / 100;
			damage = damage*gCalcDameRate;
			float CalcDameRate = (float)(gServerInfo.m_DamageRatePvM[lpObj->Class]) / 100;
...
#pragma region APPLY_EFFECT
	...
	if (lpObj->Type == OBJECT_USER && effect != 4)
	{
		...
		for (std::vector<CUSTOM_START_ITEM_DAME_INFO>::iterator it = lpInfo->ItemDameStart.begin(); it != lpInfo->ItemDameStart.end(); it++)
		{
			...
			{
				if (gServerInfo.m_DamageCustom == 1)
				{
					damage = (damage *(gServerInfo.m_DamageTotal + it->DameRate)) / gServerInfo.m_DamageBuff;
				}
			}
		}
#endif
...
bool CAttack::DecreaseArrow(LPOBJ lpObj) // OK
{
	...
	if (lpObj->Inventory[0].m_Index >= GET_ITEM(4, 0) && lpObj->Inventory[0].m_Index < GET_ITEM(5, 0) && lpObj->Inventory[0].m_Index != GET_ITEM(4, 15) && lpObj->Inventory[0].m_Slot == 0)
	{
		...
		else
		{
			...
			//gItemManager.DecreaseItemDur(lpObj,1,1); //  // Goc
		}
	}
	...
	if (lpObj->Inventory[1].m_Index >= GET_ITEM(4, 0) && lpObj->Inventory[1].m_Index < GET_ITEM(5, 0) && lpObj->Inventory[1].m_Index != GET_ITEM(4, 7) && lpObj->Inventory[1].m_Slot == 1)
	{
		...
		else
		{
			...
			//gItemManager.DecreaseItemDur(lpObj,0,1); // Goc
		}
	}
#endif
...
void CAttack::HelperSprite(LPOBJ lpObj, LPOBJ lpTarget, float* damage) // OK
{
	...
	if (lpTarget != 0 && lpTarget->Type == OBJECT_USER)
	{
		CItem* lpItem = &lpTarget->EquipInventory[12];
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	if (lpObj->Guild != 0 && lpTarget->Guild != 0)
	{
		...
		if (lpObj->Guild->Number == lpTarget->Guild->Number && gServerInfo.m_GuildPK != 0 && gDuel.CheckDuel(lpObj, lpTarget) == 0 // Pk Cung Lien Minh
			&& gCustomGreatPK.CheckPlayerJoined(lpObj, lpTarget) == 0 // pk map greatevent
			&& gBattleRoyale.CheckPlayerJoined(lpObj, lpTarget) == 0 // pk map greatevent
			&& gCustomArena.CheckPlayerJoined(lpObj, lpTarget) == 0 // pk map greatevent
			&& gThanMaChien.CheckPlayerJoined(lpObj, lpTarget) == 0 // pk map greatevent
			&& gTvTEvent.CheckPlayerJoined(lpObj, lpTarget) == 0 // pk map greatevent

			//&& gObjGetRelationShip(lpObj, lpTarget) == 0

			&& lpTarget->Map != MAP_CHAOS_CASTLE1
			&& lpTarget->Map != MAP_CHAOS_CASTLE2
			&& lpTarget->Map != MAP_CHAOS_CASTLE3
			&& lpTarget->Map != MAP_CHAOS_CASTLE4
			&& lpTarget->Map != MAP_CHAOS_CASTLE5
			&& lpTarget->Map != MAP_CHAOS_CASTLE6
			&& lpTarget->Map != MAP_CHAOS_CASTLE7
			&& lpTarget->Map != MAP_KALIMDOR
			&& lpTarget->Map != MAP_DUNES
			&& lpTarget->Map != MAP_DAU_TRUONG
			)
		{
			return 0;
		}
	...
	//----------------------// Fix event battle
	if (lpObj->Map == gBattleRoyale.EventMap &&lpTarget->Map == gBattleRoyale.EventMap &&  gBattleRoyale.GetState() == ROYAL_STATE_CLEAN)
	{

		return 0;
	}

	if (lpObj->Map == gBattleRoyale.EventMap &&lpTarget->Map == gBattleRoyale.EventMap && gBattleRoyale.GetState() == 1)
	{
	...
	if (gBattleRoyale.CheckPlayerJoined(lpObj, lpObj) && !gBattleRoyale.CheckPlayerJoined(lpTarget, lpTarget))
	{
		return 0;
	}

	if (gBattleRoyale.CheckPlayerJoined(lpTarget, lpTarget) && !gBattleRoyale.CheckPlayerJoined(lpObj, lpObj))
	{
		return 0;
	}
...
#if(SKY_EVENT)
	if (SkyEvent.NPCEnabled && lpObj->Map == MAP_ICARUS){
		return 0;
	}
#endif
...
int CAttack::GetTargetDefense(LPOBJ lpObj, LPOBJ lpTarget, WORD* effect) // OK
{
	...
	if ((GetLargeRand() % gServerInfo.GioiHanLoaiBoPhongThu) < ((lpObj->IgnoreDefenseRate + lpObj->EffectOption.AddIgnoreDefenseRate) - lpTarget->ResistIgnoreDefenseRate))
	{
		(*effect) = 1;
...
float CAttack::GetAttackDamage(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, WORD* effect, int TargetDefense) // OK
{
	...
	else
	{
		...
		if ((GetLargeRand() % 100) < ((lpObj->rDanhHieu)))
		{
			(*effect) = 7; // Tím
			damage = DamageMax;
			damage += lpObj->rDanhHieu;
		}

		if ((GetLargeRand() % 100) < ((lpObj->rTuLuyen)))
		{
			(*effect) = 6; //Trắng
			damage = DamageMax;
			damage += lpObj->rTuLuyen;

		}

		if ((GetLargeRand() % 100) < ((lpObj->rQuanHam)))
		{
			(*effect) = 5; //Hồng
			damage = DamageMax;
			damage += lpObj->rQuanHam;
		}
...
float CAttack::GetAttackDamageWizard(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, WORD* effect, int TargetDefense) // OK
{
	...
	if ((GetLargeRand() % gServerInfo.GioiHanTyLeSatThuongChiMang) < ((lpObj->CriticalDamageRate + lpObj->EffectOption.AddCriticalDamageRate) - lpTarget->ResistCriticalDamageRate))
	{
		...
		//LogAdd(LOG_RED, "[TimLoi] CriticalDamageRate Monster");
	}
