...
void CHwidManager::ConnectHwid(CG_HWID_SEND *lpMsg, LPOBJ lpObj)
{
	...
	{
		LogAdd(LOG_RED,"Invalid Version! GameServer [%s] Main [%s]", VERSION, lpMsg->DLLVersion);
		lpObj->Socket = INVALID_SOCKET;
		closesocket(lpObj->PerSocketContext->Socket);
		gObjDel(lpObj->Index);
	}

	if (gServerInfo.m_SharkGuardCRC != 0)
	{
		if (gServerInfo.m_SharkGuardCRC != lpMsg->SharkGuardCRC)
		{
			LogAdd(LOG_RED, "Invalid files SharkGuard.dll (Server: %X / Client: %X)", gServerInfo.m_SharkGuardCRC, lpMsg->SharkGuardCRC);
	...
	if (gServerInfo.m_VerifyCRC != 0)
	{
		...
		{
			LogAdd(LOG_RED, "Invalid files SharkGuard.doc (Server: %X / Client: %X)", gServerInfo.m_VerifyCRC, lpMsg->VerifyCRC);
			lpObj->Socket = INVALID_SOCKET;
			closesocket(lpObj->PerSocketContext->Socket);
			gObjDel(lpObj->Index);
			return;
