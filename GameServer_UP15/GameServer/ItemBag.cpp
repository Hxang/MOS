...
bool CItemBag::GetItem(LPOBJ lpObj, CItem* lpItem) // OK
{
	...
	for (int n = 0; n < this->m_ItemBagInfo.size(); n++)
	{
		RandomMng.AddElement(n, this->m_ItemBagInfo[n].DropRate);
	}
	...
	ITEM_BAG_INFO* lpInfo = this->GetInfo(number);
	...
	//LogAdd(LOG_EVENT, "%d ~ %d", lpInfo->Time, this->m_time);
...
{
	ITEM_BAG_INFO* lpInfo = this->GetInfo(this->number);
	...
	for (int n = 0; n < this->m_ItemDropCount; n++)
	{
		...
		if (this->GetItem(lpObj, &item) != 0)
		{
			time_t t = time(NULL);
			localtime(&t);
			DWORD iTime = (DWORD)t + this->m_time * 60;
			//LogAdd(LOG_RED, "%d ~ %d", lpInfo->Time, this->m_time);
			...
			if (this->m_time > 0)
			{
				GDCreateItemSend(lpObj->Index,
					...
					item.m_ItemOptionEx,
					item.m_SocketOption,
					item.m_SocketOptionBonus, iTime);
			}
			else
			{
				GDCreateItemSend(lpObj->Index,
