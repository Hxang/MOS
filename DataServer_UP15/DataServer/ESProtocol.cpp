...
void ExDBServerProtocolCore(int index, BYTE head, BYTE* lpMsg, int size) // OK
{
	...
	case 0x39:
		GDGuildDevoteUpdate((SDHP_GUILD_UPGRADE_DEVOTE_UPDATE*)lpMsg, index);
		break;
#endif
	case 0x50:
...
#if(CUSTOM_GUILD_UPGRADE)
		DGGuildMemberInfo(index, lpMsg->GuildName, lpMsg->Master, lpGuildMemberInfo->btStatus, lpMsg->btGuildType, (BYTE)lpGuildMemberInfo->btServer, lpGuildMemberInfo->Devote);
#else
...
void DGGuildMemberInfoRequest(SDHP_GUILDMEMBER_INFO_REQUEST* lpMsg, int index)
{
	...
	DGRelationShipListRecv(index, lpGuildInfo->dwRivalNumber, 2);
#if(CUSTOM_GUILD_UPGRADE)
	DGGuildMemberInfo(index, lpGuildInfo->szName, lpMsg->MemberID, lpGuildMemberInfo->btStatus, lpGuildInfo->btType, (BYTE)lpGuildMemberInfo->btServer, lpGuildMemberInfo->Devote);
#else
...
void GDGuildDevoteUpdate(SDHP_GUILD_UPGRADE_DEVOTE_UPDATE* lpMsg, int index)
{
	GUILD_INFO* lpGuildInfo = gGuildManager.GetMemberGuildInfo(lpMsg->Member);

	GUILD_MEMBER_INFO* lpGuildMemberInfo = gGuildManager.GetGuildMemberInfo(lpMsg->Member);
	// fix guild
	char szName[11];
	memset(szName, 0, sizeof(szName));
	memcpy(szName, lpMsg->Member, 10);
	...
	gGuildManager.SetDevote(lpMsg->Member, lpMsg->Devote, lpMsg->GuildDevote, lpMsg->GuildLevel);
	//LogAdd(LOG_RED, "Them CongHien=%d Char=[%s] , CongHienG =%d , Glevel =%d", lpMsg->Devote, lpMsg->Member, lpMsg->GuildDevote, lpMsg->GuildLevel);
...
#if(CUSTOM_GUILD_UPGRADE)
void DGGuildMemberInfo(int index, char* GuildName, char* MemberID, BYTE Status, BYTE Type, BYTE Server, int Devote)
{
...
#if(CUSTOM_GUILD_UPGRADE)
	pMsg.TotalDevote = lpGuildInfo->TotalDevote;
	pMsg.LevelDevote = lpGuildInfo->LevelDevote;
#endif
...
#if(CUSTOM_GUILD_UPGRADE)
			GuildList.Devote = lpGuildMemberInfo->Devote;
#endif
