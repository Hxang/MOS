...
void CGuildManager::Init()
{
	...
	if (gQueryManager.ExecQuery("SELECT * FROM Guild ORDER BY G_Score DESC") != 0)
#else
...
#if(CUSTOM_GUILD_UPGRADE == TRUE)
			//GuildInfo.TotalDevote = gQueryManager.GetAsInteger("TotalDevote");
			GuildInfo.TotalDevote = gQueryManager.GetAsInteger("G_Score");
			GuildInfo.LevelDevote = gQueryManager.GetAsInteger("LevelDevote");
#endif
...
#if(CUSTOM_GUILD_UPGRADE == TRUE)
			GuildMemberInfo.Devote = gQueryManager.GetAsInteger("Devote");
#endif
...
BYTE CGuildManager::AddGuild(int index, char* szGuildName, char* szMasterName, BYTE* lpMark, BYTE btType)
{
	...
	else
	{
		if (gQueryManager.GetResult(0) == 0)
		{
			...
			gQueryManager.ExecQuery("UPDATE Guild SET G_Created=GETDATE(),G_Mark=?,G_Type=%d WHERE G_Name='%s'", btType, szGuildName);

			gQueryManager.Close();

			gQueryManager.ExecQuery("UPDATE GuildMember SET G_JoinTime=GETDATE(),G_Status=%d WHERE Name='%s'", 0x80, szMasterName);
...
#if(CUSTOM_GUILD_UPGRADE == TRUE)
			GuildInfo.LevelDevote = 0;

			GuildInfo.TotalDevote = 0;
#endif
...
BYTE CGuildManager::AddGuildMember(int index, char* szGuildName, char* szGuildMember, BYTE btStatus, WORD btServer)
{
	...
	if (gQueryManager.ExecQuery("INSERT INTO GuildMember (Name,G_Name,G_Level,G_Status,G_JoinTime) VALUES ('%s','%s',%d,%d,GETDATE())", szGuildMember, szGuildName, 1, btStatus) == 0)
	{
		gQueryManager.Close();
...
bool CGuildManager::SetDevote(char* member, int Devote, int GuildDevote, int GuildLevel)
{
	...
	if (gQueryManager.ExecQuery("UPDATE GuildMember SET Devote=%d WHERE Name='%s'", Devote, member) == 0)
	{
		gQueryManager.Close();
		return false;
	}
	...
	{
		gQueryManager.Close();
		if (gQueryManager.ExecQuery("UPDATE Guild SET G_Score=%d, LevelDevote=%d WHERE Number=%d", GuildDevote, GuildLevel, lpGuildInfo->dwNumber) == 0)
		{
			gQueryManager.Close();
			return false;
		}
		...
		{
			lpGuildInfo->TotalDevote = GuildDevote;
			lpGuildInfo->LevelDevote = GuildLevel;
			lpGuildMemberInfo->Devote = Devote;
			gQueryManager.Close();
			return true;
		}
	}
...
void CGuildManager::SendGuildList()
{
	for (std::vector<GUILD_INFO>::iterator it = this->vGuildList.begin(); it != this->vGuildList.end(); it++)
	{
		DGGuildMasterListRecv(0, it->dwNumber);
	}
}
#endif