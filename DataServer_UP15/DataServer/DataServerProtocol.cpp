...
void DataServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x05:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
				...
				case 0x75:
					...
					break;
				case 0x76:
					gWarehouse.GDWarehouseGuildConsult((SDHP_WAREHOUSE_GUILD_STATUS_RECV*)lpMsg,index);
					break;
				case 0x77:
					gWarehouse.GDWarehouseGuildUpdate((SDHP_WAREHOUSE_GUILD_STATUS_UPDATE_RECV*)lpMsg);
					break;
			}
			break;
		case 0x07:
		...
		case 0x3C:
			...
			break;
		case 0x3B:
			GDRankingBosSaveRecv((SDHP_RANKING_BOS_SAVE_RECV*)lpMsg);
			break;
		case 0x3D:
		...
		case 0x56:
			...
			break;
		case 0x58:
			GDRankingCustomGreatPKSaveRecv((SDHP_RANKING_CUSTOM_GREATPK_SAVE_RECV*)lpMsg);
			break;
		...
		case 0xD9:
			...
			{
			case 0x01:
				GDGetSkinIsBuy((GSSENDDS_GETLISTISBUYSKIN*)lpMsg, index); // OK
				break;
				//==Save Skin Da MUa
			case 0x02:
				GDSaveSkinBuy((GSSENDDS_GETLISTISBUYSKIN*)lpMsg, index); // OK
				break;
			case 0x07:
				GDSaveTheGiftRecv((THEGIFT_GD_SAVE_DATA*)lpMsg); // Only add in the case 0xD9
				break;
#if (JEWELBANKVER2)
			case 0x21: //==Custom Item Bank
				GetDataCustomItemBank((GSSENDDS_COUNT_ITEMBANK*)lpMsg, index); // 
				break;
			case 0x22: //==Save Bank
				SaveDataCustomItemBank((GSSENDDS_COUNT_ITEMBANK*)lpMsg, index); // 
				break;
#endif
...
void GDCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg,int index) // OK
{
	...
	else
	{
...
#endif
#if(DANHHIEU_NEW)
		pMsg.rQuanHam = gQueryManager.GetAsInteger("rQuanHam");
#endif
		...
		pMsg.CTCTime = (DWORD)gQueryManager.GetAsInteger("CTCTime");

		pMsg.CTCRegDay = (DWORD)gQueryManager.GetAsInteger("CTCRegDay");
		...
		//==========Select Custom CB
		gQueryManager.ExecQuery("SELECT UserSkinPick from Character where AccountID='%s' AND Name='%s'", lpMsg->account, lpMsg->name);

		gQueryManager.Fetch();

		pMsg.mUserSkinPick = (DWORD)gQueryManager.GetAsInteger("UserSkinPick");

		gQueryManager.Close();


#if(B_HON_HOAN)
		gQueryManager.ExecQuery("SELECT HonHoan from Character where AccountID='%s' AND Name='%s'", lpMsg->account, lpMsg->name);

		gQueryManager.Fetch();

		pMsg.CapDoHonHoan = (BYTE)gQueryManager.GetAsInteger("HonHoan");
		gQueryManager.Close();
#endif

#if(FLAG_SKIN)
		gQueryManager.ExecQuery("SELECT * FROM Character WHERE AccountID='%s' AND Name='%s'", lpMsg->account, lpMsg->name);
		gQueryManager.Fetch();
...
#endif
#if(B_MOCNAP)
		//===GetTongNap //cai nay co the thay doi tuy theo src
		char szQuery[1024];
		sprintf(szQuery, "SELECT SUM(TienNap) as TongNap FROM DataNapGame WHERE Account = '%s' AND status = 1", lpMsg->account);
...
void GDCharacterInfoSaveRecv(SDHP_CHARACTER_INFO_SAVE_RECV* lpMsg) // OK
{
	...
	//==CB Custom New
	gQueryManager.ExecQuery("UPDATE Character SET UserSkinPick='%d' WHERE AccountID='%s' AND Name='%s'", lpMsg->mUserSkinPick, lpMsg->account, lpMsg->name);
	gQueryManager.Close();
#if(DANHHIEU_NEW)
	gQueryManager.ExecQuery("UPDATE Character SET rDanhHieu=%d WHERE AccountID='%s' AND Name='%s'", lpMsg->rDanhHieu, lpMsg->account, lpMsg->name);
	
	gQueryManager.Close();
	LogAdd(LOG_BLACK,"UPDATE Character SET rDanhHieu=%d WHERE AccountID='%s' AND Name='%s'", lpMsg->rDanhHieu, lpMsg->account, lpMsg->name);

#endif
#if(DANHHIEU_NEW)
	gQueryManager.ExecQuery("UPDATE Character SET rQuanHam=%d WHERE AccountID='%s' AND Name='%s'", lpMsg->rQuanHam, lpMsg->account, lpMsg->name);

	gQueryManager.Close();
	LogAdd(LOG_BLACK, "UPDATE Character SET rQuanHam=%d WHERE AccountID='%s' AND Name='%s'", lpMsg->rQuanHam, lpMsg->account, lpMsg->name);

#endif
#if(TULUYEN_NEW)
	gQueryManager.ExecQuery("UPDATE Character SET rTuLuyen=%d WHERE AccountID='%s' AND Name='%s'", lpMsg->rTuLuyen, lpMsg->account, lpMsg->name);
	gQueryManager.Close();
...
#if(B_HON_HOAN)
	gQueryManager.ExecQuery("UPDATE Character SET HonHoan=%d WHERE AccountID='%s' AND Name='%s'", lpMsg->CapDoHonHoan, lpMsg->account, lpMsg->name);
	gQueryManager.Close();
	...
	gQueryManager.Close();
	gQueryManager.ExecQuery("UPDATE Character SET Flag=%d WHERE AccountID='%s' AND Name='%s'", lpMsg->Flag, lpMsg->account, lpMsg->name);
	gQueryManager.Close();
	gQueryManager.ExecQuery("UPDATE Character SET CheckFlag=%d WHERE AccountID='%s' AND Name='%s'", lpMsg->CheckFlag, lpMsg->account, lpMsg->name);
	gQueryManager.Close();
	//Flagrong
#endif
#if(B_MOCNAP)
	gQueryManager.ExecQuery("UPDATE MEMB_INFO SET NhanMocNap='%d' WHERE memb___id='%s'", lpMsg->NhanMocNap, lpMsg->account);
	gQueryManager.Close();
#endif
	#else
...
void GDRankingCustomGreatPKSaveRecv(SDHP_RANKING_CUSTOM_GREATPK_SAVE_RECV* lpMsg) // OK
{
	gQueryManager.ExecQuery("EXEC WZ_CustomGreatPKRanking '%s','%s','%d','%d','%d'", lpMsg->account, lpMsg->name, lpMsg->number, lpMsg->score, lpMsg->rank);
	gQueryManager.Fetch();
	gQueryManager.Close();
}
...
void GDCustomRankingRecv(SDHP_CUSTOM_RANKING_RECV* lpMsg, int index) // OK
{
	...
	if (gQueryManager.ExecQuery("EXEC WZ_CustomRanking %d", lpMsg->type) != 0)
	{
		...
		{
			gQueryManager.GetAsString("VALUE1", info.Value1, sizeof(info.Value1));

			gQueryManager.GetAsString("VALUE2", info.Value2, sizeof(info.Value2));

			gQueryManager.GetAsString("VALUE3", info.Value3, sizeof(info.Value3));
...
void GDCustomGHRSRecv(SDHP_CUSTOM_GHRS_RECV* lpMsg, int index) // OK
{
	...
	if (gQueryManager.ExecQuery("select top 1 MasterResetCount, ResetCount from GHRS_top1 where time = %d order by MasterResetCount DESC, ResetCount DESC", lpMsg->time) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
		if (gQueryManager.ExecQuery("select TOP 1 MasterResetCount, ResetCount from Character where CtlCode = 0 Order by MasterResetCount DESC, ResetCount DESC, cLevel DESC, Experience DESC, Money DESC") != 0 && gQueryManager.Fetch() != SQL_NO_DATA) {
...
void GDGetSkinIsBuy(GSSENDDS_GETLISTISBUYSKIN* lpMsg, int index) // OK
{
	...
	if (gQueryManager.ExecQuery("Select SkinIndex,StatusBuy from SkinModel where AccountID='%s'", lpMsg->AccountID) != 0)
	{
		while (gQueryManager.Fetch() != SQL_NO_DATA)
		{
...
void GDRankingBosSaveRecv(SDHP_RANKING_BOS_SAVE_RECV* lpMsg)
{
	gQueryManager.ExecQuery("EXEC WZ_RankingBos '%s','%s','%d'", lpMsg->account, lpMsg->name, lpMsg->score);
	gQueryManager.Fetch();
	gQueryManager.Close();
}
#if(JEWELBANKVER2)
void GetDataCustomItemBank(GSSENDDS_COUNT_ITEMBANK* lpMsg, int aIndex)
{
	BYTE send[4096];
	...
	for (int n = 0; n < lpMsg->Count; n++)
	{
		...
		//==  lpInfo->ItemCount == Level Get
		if (gQueryManager.ExecQuery("SELECT ItemCount,AutoPick FROM CustomItemBank WHERE AccountID='%s' AND ItemIndex='%d' AND ItemLevel='%d' ", lpMsg->AccountID, lpInfo->ItemIndex, lpInfo->ItemLevel) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
		{
			gQueryManager.Close();
			gQueryManager.ExecQuery("INSERT INTO CustomItemBank (AccountID,ItemIndex,ItemLevel,ItemCount,AutoPick) VALUES ('%s',%d,%d,%d,0)", lpMsg->AccountID, lpInfo->ItemIndex, lpInfo->ItemLevel, 0);
			gQueryManager.Close();
		...
		else
		{
			info.ItemIndex = lpInfo->ItemIndex;
			info.ItemLevel = lpInfo->ItemLevel;
			info.ItemCount = gQueryManager.GetAsInteger("ItemCount");
			info.AutoPick = gQueryManager.GetAsInteger("AutoPick");
			gQueryManager.Close();
		}
...
void SaveDataCustomItemBank(GSSENDDS_COUNT_ITEMBANK* lpMsg, int aIndex)
{
	//LogAdd(LOG_RED, "SaveDataCustomItemBank %s", lpMsg->AccountID);
	for (int n = 0; n < lpMsg->Count; n++)
	{
		USER_BANK_DATA* lpInfo = (USER_BANK_DATA*)(((BYTE*)lpMsg) + sizeof(GSSENDDS_COUNT_ITEMBANK)+(sizeof(USER_BANK_DATA)* n));

		gQueryManager.ExecQuery("Update CustomItemBank SET ItemCount = '%d', AutoPick='%d' WHERE AccountID = '%s' And ItemIndex = '%d' And ItemLevel='%d' ", lpInfo->ItemCount, lpInfo->AutoPick, lpMsg->AccountID, lpInfo->ItemIndex, lpInfo->ItemLevel);
		gQueryManager.Fetch();
		gQueryManager.Close();
	}
}
