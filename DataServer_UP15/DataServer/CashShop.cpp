...
int CCashShop::FixCoinLag(DWORD Coin) // OK
{
	int mCoin = 0;
	if ((int)Coin > 0 && (int)Coin < 2147483647)
	{
		mCoin = (int)Coin;
	}
	if (Coin >= 0x7FFFFFFF)
...
void CCashShop::GDCashShopAddPointSaveRecv(SDHP_CASH_SHOP_ADD_POINT_SAVE_RECV* lpMsg) // OK Cong Coin
{
#if(DATASERVER_UPDATE>=501)
	...
	//========Fix Coin
	lpMsg->AddWCoinC = gCashShop.FixCoinLag(lpMsg->AddWCoinC);

	lpMsg->AddWCoinP = gCashShop.FixCoinLag(lpMsg->AddWCoinP);

	lpMsg->AddGoblinPoint = gCashShop.FixCoinLag(lpMsg->AddGoblinPoint);

	lpMsg->AddRuud = gCashShop.FixCoinLag(lpMsg->AddRuud);
	...
	if (gQueryManager.ExecQuery("SELECT WCoinC,WCoinP,GoblinPoint,Ruud FROM CashShopData WHERE AccountID='%s'", TargetAccount) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		...
		gQueryManager.ExecQuery("INSERT INTO CashShopData (AccountID,WCoinC,WCoinP,GoblinPoint,Ruud) VALUES ('%s',%d,%d,%d,%d)", TargetAccount, gCashShop.FixCoinLag(lpMsg->AddWCoinC), gCashShop.FixCoinLag(lpMsg->AddWCoinP), gCashShop.FixCoinLag(lpMsg->AddGoblinPoint), gCashShop.FixCoinLag(lpMsg->AddRuud));
	...
	else
	{
		...
		//==== Fixc
		WCoinC = gCashShop.FixCoinLag(WCoinC);

		WCoinP = gCashShop.FixCoinLag(WCoinP);

		GoblinPoint = gCashShop.FixCoinLag(GoblinPoint);


		gQueryManager.ExecQuery("UPDATE CashShopData SET WCoinC=%d,WCoinP=%d,GoblinPoint=%d,Ruud=%d WHERE AccountID='%s'", gCashShop.FixCoinLag(WCoinC + lpMsg->AddWCoinC), gCashShop.FixCoinLag(WCoinP + lpMsg->AddWCoinP), gCashShop.FixCoinLag(GoblinPoint + lpMsg->AddGoblinPoint), gCashShop.FixCoinLag(Ruud + lpMsg->AddRuud), TargetAccount);
...
void CCashShop::GDCashShopSubPointSaveRecv(SDHP_CASH_SHOP_SUB_POINT_SAVE_RECV* lpMsg) // OK
{
	...
	else
	{
		...
		RuudPoint = gCashShop.FixCoinLag(RuudPoint);

		lpMsg->SubWCoinC = gCashShop.FixCoinLag(lpMsg->SubWCoinC);

		lpMsg->SubWCoinP = gCashShop.FixCoinLag(lpMsg->SubWCoinP);

		lpMsg->SubGoblinPoint = gCashShop.FixCoinLag(lpMsg->SubGoblinPoint);

		lpMsg->SubRuud = gCashShop.FixCoinLag(lpMsg->SubRuud);

		int sqlWcoinC = (int)((lpMsg->SubWCoinC > WCoinC) ? 0 : (WCoinC - lpMsg->SubWCoinC));
		int sqlWcoinP = (int)((lpMsg->SubWCoinP > WCoinP) ? 0 : (WCoinP - lpMsg->SubWCoinP));
		int sqlGobinP = (int)((lpMsg->SubGoblinPoint > GoblinPoint) ? 0 : (GoblinPoint - lpMsg->SubGoblinPoint));
		int sqlRuud = (int)((lpMsg->SubRuud > RuudPoint) ? 0 : (RuudPoint - lpMsg->SubRuud));

		gQueryManager.ExecQuery("UPDATE CashShopData SET WCoinC=%d, WCoinP=%d, GoblinPoint=%d, Ruud=%d WHERE AccountID='%s'", gCashShop.FixCoinLag(sqlWcoinC), gCashShop.FixCoinLag(sqlWcoinP), gCashShop.FixCoinLag(sqlGobinP), gCashShop.FixCoinLag(sqlRuud), TargetAccount);

		gQueryManager.Close();

		if (lpMsg->SubWCoinC > 0)
		{
			gQueryManager.ExecQuery("INSERT INTO LOG_CREDITOS (login,valor,tipo) VALUES ('%s',%d,%d)", TargetAccount, ((lpMsg->SubWCoinC > WCoinC) ? 0 : (-lpMsg->SubWCoinC)), 1);

			gQueryManager.Close();
		}
		if (lpMsg->SubWCoinP > 0)
		{
			gQueryManager.ExecQuery("INSERT INTO LOG_CREDITOS (login,valor,tipo) VALUES ('%s',%d,%d)", TargetAccount, ((lpMsg->SubWCoinP > WCoinP) ? 0 : (-lpMsg->SubWCoinP)), 2);

			gQueryManager.Close();
		}
		if (lpMsg->SubGoblinPoint > 0)
		{
			gQueryManager.ExecQuery("INSERT INTO LOG_CREDITOS (login,valor,tipo) VALUES ('%s',%d,%d)", TargetAccount, ((lpMsg->SubGoblinPoint > GoblinPoint) ? 0 : (-lpMsg->SubGoblinPoint)), 3);

			gQueryManager.Close();
		}
		if (lpMsg->SubRuud > 0)
		{
			gQueryManager.ExecQuery("INSERT INTO LOG_CREDITOS (login,valor,tipo) VALUES ('%s',%d,%d)", TargetAccount, ((lpMsg->SubRuud > RuudPoint) ? 0 : (-lpMsg->SubRuud)), 4);
