...
bool CSocketManagerUdp::Start(WORD port) // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketManagerUdp] WSASocket() Không thành công với lỗi: %d",WSAGetLastError());
		this->Clean();
		return 0;
	}
	...
	{
		LogAdd(LOG_RED,"[SocketManagerUdp] bind() Không thành công với lỗi: %d",WSAGetLastError());
		this->Clean();
		return 0;
	}
	...
	{
		LogAdd(LOG_RED,"[SocketManagerUdp] CreateThread() Không thành công với lỗi: %d",GetLastError());
		this->Clean();
		return 0;
	}
...
bool CSocketManagerUdp::Connect(char* IpAddress,WORD port) // OK
{
	...
	if(this->m_SocketAddr.sin_addr.s_addr == INADDR_NONE)
	{
		...
		if(host == 0)
		{
			LogAdd(LOG_RED,"[SocketManagerUdp] gethostbyname() Không thành công với lỗi: %d",WSAGetLastError());
			this->Clean();
			return 0;
		}
...
bool CSocketManagerUdp::DataRecv() // OK
{
	...
	while(true)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketManagerUdp] Lỗi tiêu đề giao thức (Tiêu đề: %x)",lpMsg[count]);
			memset(this->m_RecvBuff,0,sizeof(this->m_RecvBuff));
			this->m_RecvSize = 0;
			return 0;
		}
		...
		{
			LogAdd(LOG_RED,"[SocketManagerUdp] Lỗi kích thước giao thức (Tiêu đề: %x, Kích thước: %d, Đầu: %x)",header,size,head);
			memset(this->m_RecvBuff,0,sizeof(this->m_RecvBuff));
			this->m_RecvSize = 0;
			return 0;
		}
...
bool CSocketManagerUdp::DataSend(BYTE* lpMsg,int size) // OK
{
	...
	{
		LogAdd(LOG_RED, "[SocketManagerUdp] Kích thước msg tối đa (Kích thước: %d)", size);
		memset(this->m_SendBuff,0,sizeof(this->m_SendBuff));
		this->m_SendSize = 0;
		return 0;
	}
	...
	if(result == SOCKET_ERROR)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketManagerUdp] sendto() Không thành công với lỗi: %d",WSAGetLastError());
			memset(this->m_SendBuff,0,sizeof(this->m_SendBuff));
			this->m_SendSize = 0;
			return 0;
		}
...
DWORD WINAPI CSocketManagerUdp::ServerRecvThread(CSocketManagerUdp* lpSocketManagerUdp) // OK
{
	while(true)
	{
		...
		if(result == SOCKET_ERROR)
		{
			LogAdd(LOG_RED,"[SocketManagerUdp] recvfrom() Không thành công với lỗi: %d",GetLastError());
