...
bool CSocketManager::Start(WORD port) // OK
{
	...
	LogAdd(LOG_BLACK,"[SocketManager] Khởi Động Port [%d] Thành Công!",this->m_port);
	return 1;
}
...
bool CSocketManager::CreateListenSocket() // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketManager] WSASocket() Không thành công với lỗi: %d",WSAGetLastError());
		return 0;
	}
	...
	{
		//LogAdd(LOG_RED,"[SocketManager] bind() Không thành công với lỗi: %d",WSAGetLastError());
		LogAdd(LOG_RED, "Lỗi: %d có thể trùng Port.", WSAGetLastError());
		LogAdd(LOG_RED, "Hoặc có JS khác đang chạy chưa được ngắt!", WSAGetLastError());
		return 0;
	}

	if(listen(this->m_listen,5) == SOCKET_ERROR)
	{
		LogAdd(LOG_RED,"[SocketManager] listen() Không thành công với lỗi: %d",WSAGetLastError());
		return 0;
	}
...
bool CSocketManager::CreateCompletionPort() // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketManager] socket() Không thành công với lỗi: %d",WSAGetLastError());
		return 0;
	}

	if((this->m_CompletionPort=CreateIoCompletionPort((HANDLE)socket,0,0,0)) == 0)
	{
		LogAdd(LOG_RED,"[SocketManager] CreateIoCompletionPort() Không thành công với lỗi: %d",GetLastError());
		closesocket(socket);
		return 0;
	}
...
bool CSocketManager::CreateAcceptThread() // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketManager] CreateThread() Không thành công với lỗi: %d",GetLastError());
		return 0;
	}

	if(SetThreadPriority(this->m_ServerAcceptThread,THREAD_PRIORITY_HIGHEST) == 0)
	{
		LogAdd(LOG_RED,"[SocketManager] SetThreadPriority() Không thành công với lỗi: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::CreateServerQueue() // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketManager] CreateSemaphore() Không thành công với lỗi: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while(true)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketManager] Lỗi tiêu đề giao thức (Chỉ mục: %d, Tiêu đề: %x)",index,lpMsg[count]);
			return 0;
		}

		if(size < 3 || size > MAX_MAIN_PACKET_SIZE)
		{
			LogAdd(LOG_RED, "[SocketManager] Lỗi kích thước giao thức (Chỉ mục: %d, Tiêu đề: %x, Kích thước: %d, Đầu: %x)", index, header, size, head);
			return 0;
		}
...
bool CSocketManager::DataSend(int index,BYTE* lpMsg,int size) // OK
{
	...
	{
		LogAdd(LOG_RED, "[SocketManager] Kích thước msg tối đa (Loại: 1, Chỉ mục: %d, Kích thước: %d)", index, size);
		this->m_critical.unlock();
		return 0;
	}
	...
	if(lpIoContext->IoSize > 0)
	{
		...
		{
			LogAdd(LOG_RED, "[SocketManager] Kích thước msg tối đa (Loại: 2, Chỉ mục: %d, Kích thước: %d)", index, (lpIoContext->IoSideBuffer.size + size));
	...
	if(WSASend(lpServerManager->m_socket,&lpIoContext->wsabuf,1,&SendSize,Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketManager] WSASend() Không thành công với lỗi: %d",WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return 0;
		}
...
void CSocketManager::Disconnect(int index) // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketManager] closesocket() Không thành công với lỗi: %d",WSAGetLastError());
		this->m_critical.unlock();
		return;
	}
...
void CSocketManager::OnRecv(int index,DWORD IoSize,IO_RECV_CONTEXT* lpIoContext) // OK
{
	...
	if(WSARecv(lpServerManager->m_socket,&lpIoContext->wsabuf,1,&RecvSize,&Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSA_IO_PENDING)
		{
			LogAdd(LOG_RED,"[SocketManager] WSARecv() Không thành công với lỗi: %d",WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return;
		}
...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		...
		if(socket == SOCKET_ERROR && WSAGetLastError() != WSAEWOULDBLOCK)
		{
			...
			LogAdd(LOG_RED,"[SocketManager] WSAAccept() Không thành công với lỗi: %d",WSAGetLastError());
...
DWORD WINAPI CSocketManager::ServerWorkerThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		if(GetQueuedCompletionStatus(lpSocketManager->m_CompletionPort,&IoSize,&index,&lpOverlapped,INFINITE) == 0)
		{
			...
			{
				lpSocketManager->m_critical.lock();
				LogAdd(LOG_RED,"[SocketManager] GetQueuedCompletionStatus() Không thành công với lỗi: %d",GetLastError());
				lpSocketManager->m_critical.unlock();
				return 0;
			}
		}
...
DWORD WINAPI CSocketManager::ServerQueueThread(CSocketManager* lpSocketManager) // OK
{
	while(true)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketManager] WaitForSingleObject() Không thành công với lỗi: %d",GetLastError());
			break;
		}
