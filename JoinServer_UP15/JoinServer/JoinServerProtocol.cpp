...
void GJRegisterAccountRecv(SDHP_REGISTER_ACCOUNT_SEND* lpMsg, int index)
{
	...
	if (CheckTextSyntax(lpMsg->password, sizeof(lpMsg->password)) == 0)
	{
		pMsg.result = CREATE_ACCOUNT_FAIL_RESIDENT;
		gSocketManager.DataSend(index, (BYTE*)&pMsg, pMsg.header.size);
		LogAdd(LOG_RED, "Account %s Register fail Syntax!");
	...
	if (CheckTextSyntax(lpMsg->personalcode, sizeof(lpMsg->personalcode)) == 0)
	{
		pMsg.result = CREATE_ACCOUNT_FAIL_RESIDENT;
		gSocketManager.DataSend(index, (BYTE*)&pMsg, pMsg.header.size);
		LogAdd(LOG_RED, "Account %s Register fail Syntax!");
	...
	if (CheckTextSyntax(lpMsg->Email, sizeof(lpMsg->Email)) == 0)
	{
		pMsg.result = CREATE_ACCOUNT_FAIL_RESIDENT;
		gSocketManager.DataSend(index, (BYTE*)&pMsg, pMsg.header.size);
		LogAdd(LOG_RED, "Account %s Register fail Syntax!");
	...
	if (MD5Encryption == 0)
	{
		...
		{
			gQueryManager.Close();
			if (gQueryManager.ExecQuery("INSERT INTO dbo.MEMB_INFO(memb___id, memb__pwd, memb_name, sno__numb, mail_chek, bloc_code, ctl1_code) VALUES ('%s', '%s', '%s', '%s', 1, 0, 1)",
				lpMsg->account,
				lpMsg->password,
				"JoinServer",
				lpMsg->personalcode /*,
